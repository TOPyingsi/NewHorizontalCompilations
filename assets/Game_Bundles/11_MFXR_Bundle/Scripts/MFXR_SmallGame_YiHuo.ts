...
@ccclass('MFXR_SmallGame_YiHuo')
export class MFXR_SmallGame_YiHuo extends Component {
    @property(Label)
    timelb: Label = null;
    @property(Sprite)
    jdt: Sprite = null;
    ...
    start() {
        MFXR_GameManager.Instance.OpenText("疑惑")
    ...
    Move(touch: EventTouch) {
        ...
        let y = this.node.getChildByName("拖动框").position.y + touch.getUIDelta().y;
        if (y < -280) y = -280;
        if (y > 280) y = 280;
        this.node.getChildByName("拖动框").position = v3(0, y, 0);
        this.index += Math.abs(touch.getUIDelta().y);
