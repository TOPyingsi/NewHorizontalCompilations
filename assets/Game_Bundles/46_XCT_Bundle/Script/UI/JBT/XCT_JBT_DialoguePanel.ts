...
@ccclass('XCT_JBT_DialoguePanel')
export class XCT_JBT_DialoguePanel extends XCT_BasePanel {
    ...
    init(isNeedShowTable: boolean = true) {
        ...
        if (XCT_JBT_DataManager.Instance.needShowTakeoutCome) {
            this.showTakeoutCom();
        }

        this.isCatClicked = false;

        XCT_JBT_DataManager.Instance.isCatDriveAway = false;
        if (XCT_JBT_DataManager.Instance.playerData.currentDay == 1 || XCT_JBT_DataManager.Instance.playerData.isAdoptedCat) {
    ...
    onShowTableItem() {
        ...
        this.btnTakeout.active = XCT_JBT_DataManager.Instance.playerData.isTakeOutProjectSet;
        this.btnTakeout.getChildByName("nodeVideo").active = !XCT_JBT_DataManager.Instance.playerData.isTakeOutProjectOpen
    }
    ...
    onClickTakeout() {
        ...
        if (XCT_JBT_DataManager.Instance.playerData.isTakeOutProjectOpen) {
            ...
            this.btnTakeout.getChildByName("nodeOrderCome").active = false;
            XCT_UIManager.Instance.showPanel(XCT_UIPanel.JBT_TakeoutPanel);
        }
        else {
            XCT_UIManager.Instance.showPanel(XCT_UIPanel.JBT_TipPanel, null, () => {
                EventManager.Scene.emit(XCT_Events.JBT_ShowTip_OpenTakeout);
            })
        }
    ...
    showTakeoutCom() {
        if (XCT_JBT_DataManager.Instance.playerData.isTakeOutProjectOpen) {
            if (XCT_JBT_DataManager.Instance.isShowingTakeoutPanel) return;
    ...
    //         .call(() => {
    //             if (!XCT_JBT_DataManager.Instance.isTakeout) {
    ...
    onBtnDriveAwayClick() {
        XCT_JBT_DataManager.Instance.isCatDriveAway = true;
        let dialogue2 = this.btnCat.getChildByName("dialogue2")
    ...
    onBtnAdoptClick() {
        XCT_JBT_DataManager.Instance.playerData.isAdoptedCat = true;
        this.btnCat.active = false;
    }
    ...
    showDishNode() {
        this.cb = ()=>{
            ...
            tween(this.dishNode)
                ...
                .call(() => {
                    if(XCT_JBT_DataManager.Instance.isNeedShowDragTutorial){
    ...
    dishDragEnd() {
        if(XCT_JBT_DataManager.Instance.isNeedShowDragTutorial){
            ...
            this.dargTurtrialAnim.active = false;
            XCT_JBT_DataManager.Instance.isNeedShowDragTutorial = false;
        }
        if (XCT_JBT_DataManager.Instance.isTakeout) {

        }
        else {
            EventManager.Scene.emit(XCT_Events.JBT_Continue_Customer_EndDialog);
        }
    ...
    addListener() {
        ...
        EventManager.on(XCT_Events.JBT_Show_Add_Tip, this.onShowAddTip, this);

        // EventManager.on(XCT_Events.JBT_Show_Dish, this.showDish, this);
        // EventManager.on(XCT_Events.JBT_Take_Dish, this.takeDish, this);

        EventManager.on(XCT_Events.JBT_Takeout_Open, this.takeoutOpen, this);

        EventManager.on(XCT_Events.JBT_New_Takeout_Order, this.showTakeoutCom, this);
        EventManager.on(XCT_Events.JBT_Hide_New_Takeout_Order, this.hideTakeoutCom, this);
        EventManager.on(XCT_Events.JBT_Show_Dish_Node, this.showDishNode, this);
    ...
    removeListener() {
        ...
        EventManager.off(XCT_Events.JBT_Show_Add_Tip, this.onShowAddTip, this);
        // EventManager.off(XCT_Events.JBT_Show_Dish, this.showDish, this);
        // EventManager.off(XCT_Events.JBT_Take_Dish, this.takeDish, this);
        EventManager.off(XCT_Events.JBT_New_Takeout_Order, this.showTakeoutCom, this);
        EventManager.off(XCT_Events.JBT_Hide_New_Takeout_Order, this.hideTakeoutCom, this);
        EventManager.off(XCT_Events.JBT_Takeout_Open, this.takeoutOpen, this);
        EventManager.off(XCT_Events.JBT_Show_Dish_Node, this.showDishNode, this);
