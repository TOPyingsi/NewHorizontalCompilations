...
@ccclass('XCT_JBT_ShopPanel')
export class XCT_JBT_ShopPanel extends XCT_BasePanel {
    ...
    init(isCooking: boolean = false) {
        ...
        XCT_JBT_DataManager.Instance.isShowingShopPanel = true;
    ...
    initShopItem() {
        ...
        XCT_JBT_DataManager.Instance.ingredientsConfig.forEach((shopConfig, index) => {
            let ingredientName = shopConfig.name;
            const hasIngredient = XCT_JBT_DataManager.Instance.playerData.stock[ingredientName] !== undefined && XCT_JBT_DataManager.Instance.playerData.stock[ingredientName] > 0; // 假设库存检查结果
            if (!hasIngredient) {
                ...
                let color = XCT_JBT_DataManager.Instance.playerData.money >= shopConfig.price ? Color.WHITE : Color.RED;
    ...
    updateShopItem() {
        this.shopItemContainer.children.forEach((shopItem) => {
            let color = XCT_JBT_DataManager.Instance.playerData.money >= this.currentData.get(shopItem.name) ? Color.WHITE : Color.RED;
            shopItem.getChildByName('btnBuy').getChildByName('lblPrice').getComponent(Label).color = color;
        })
    }
    ...
    onClickBuy(ingredientName: string) {
        ...
        if (XCT_JBT_DataManager.Instance.playerData.money < XCT_JBT_DataManager.Instance.ingredientsConfigObject[ingredientName].price) {
        ...
        // 进货
        if (XCT_JBT_DataManager.Instance.playerData.stock[ingredientName]) {
            XCT_JBT_DataManager.Instance.playerData.stock[ingredientName]++;
        } else {
            XCT_JBT_DataManager.Instance.playerData.stock[ingredientName] = 1;
        }
        XCT_JBT_DataManager.Instance.playerData.money -= XCT_JBT_DataManager.Instance.ingredientsConfigObject[ingredientName].price;
        EventManager.Scene.emit(XCT_Events.JBT_Update_Money);
        EventManager.Scene.emit(XCT_Events.JBT_Update_Ingredient);
    ...
    onClickNextDay() {
        ...
        XCT_UIManager.Instance.showPanel(XCT_UIPanel.PassPanel, {
            tipConfig: XCT_JBT_DataManager.Instance.tipConfig,
            currentDay: XCT_JBT_DataManager.Instance.playerData.currentDay
            , passDayCb: () => {
                XCT_JBT_DataManager.Instance.passDay();
            }
        }, () => {
            XCT_UIManager.Instance.hidePanel(XCT_UIPanel.JBT_ShopPanel);
        ...
        XCT_JBT_DataManager.Instance.isHadShowShop = false;
    }
    ...
    onClickReturn() {
        ...
        XCT_JBT_DataManager.Instance.isShowingShopPanel = false;
    }
    ...
    // 注册事件监听
    addListener() {
        EventManager.on(XCT_Events.JBT_Update_Money, this.updateShopItem, this)
    ...
    // 注销事件监听
    removeListener() {
        EventManager.off(XCT_Events.JBT_Update_Money, this.updateShopItem, this)
