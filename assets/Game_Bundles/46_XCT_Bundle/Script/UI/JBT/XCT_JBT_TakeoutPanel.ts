...
import { XCT_JBT_OrderStatus, XCT_JBT_TakeoutOrder, XCT_JBT_DataManager } from '../../Manager/XCT_JBT_DataManager';
...
@ccclass('XCT_JBT_TakeoutPanel')
export class XCT_JBT_TakeoutPanel extends XCT_BasePanel {
    ...
    init() {
        ...
        XCT_JBT_DataManager.Instance.isShowingTakeoutPanel = true;
        super.init();
        this.refreshPanel();
    ...
    refreshPanel() {
        ...
        this.updateStars(XCT_JBT_DataManager.Instance.playerData.score);

        // 获取订单列表并刷新
        const orders: XCT_JBT_TakeoutOrder[] = XCT_JBT_DataManager.Instance.currentTakeoutOrders;

        this.nodeNoOrder.active = orders.length === 0;
        //将orders里的内容倒序排列到一个新数组里，再刷新列表
        const newOrders: XCT_JBT_TakeoutOrder[] = [];
    ...
    // 刷新订单列表
    refreshOrderList(orders: XCT_JBT_TakeoutOrder[]) {
        // 清除现有订单
        ...
        orders.forEach(order => {
           ...
           XCT_JBT_DataManager.Instance.lastTakeoutOrderIds = XCT_JBT_DataManager.Instance.currentTakeoutOrders.map(order => order.id);
    }

    // 更新单个订单项
    updateOrderItem(itemNode: Node, order: XCT_JBT_TakeoutOrder) {
        let lastTakeoutOrderIds = XCT_JBT_DataManager.Instance.lastTakeoutOrderIds;
        ...
        switch (order.status) {
            case XCT_JBT_OrderStatus.UNHANDLED:
                if (order.missingStockIngredients.length > 0) {
                ...
                break;
            case XCT_JBT_OrderStatus.TIMEOUT:
                nodeTimeout.active = true;
                break;
            case XCT_JBT_OrderStatus.COOKED:
                nodeCookEnd.active = true;
                break;
            case XCT_JBT_OrderStatus.DELIVERED:
                nodeDelivered.active = true;
                break;
            case XCT_JBT_OrderStatus.REFUSED:
    ...
    // 处理接单
    handleVideo(orderData: XCT_JBT_TakeoutOrder, missLabelNode: Node) {
        XCT_AudioManager.getInstance().playSound("点击");
        Banner.Instance.ShowVideoAd(() => {
            // 假设视频播放完成
            if (XCT_JBT_DataManager.Instance.playerData.stock[orderData.missingStockIngredients[0]]) {
                XCT_JBT_DataManager.Instance.playerData.stock[orderData.missingStockIngredients[0]]++;
            } else {
                XCT_JBT_DataManager.Instance.playerData.stock[orderData.missingStockIngredients[0]] = 1;
            }
        ...
        // XCT_JBT_DataManager.Instance.updateOrderStatus(orderId, OrderStatus.COOKED);
    }

    // 处理接单
    handleAcceptOrder(orderData: XCT_JBT_TakeoutOrder) {
        XCT_AudioManager.getInstance().playSound("点击");
        XCT_JBT_DataManager.Instance.resetTakout(orderData);
        XCT_JBT_DataManager.Instance.takeoutNotes.push([orderData.content, `备注: ${orderData.remark}`])
    ...
    // 处理拒单
    handleRefuseOrder(orderData: XCT_JBT_TakeoutOrder) {
        XCT_AudioManager.getInstance().playSound("点击");
        // 实际项目中可能需要确认逻辑
        orderData.status = XCT_JBT_OrderStatus.REFUSED;
        // XCT_JBT_DataManager.Instance.updateOrderStatus(orderId, OrderStatus.TIMEOUT);
        this.refreshPanel(); // 刷新面板
    }

    onClickReturn() {
        XCT_AudioManager.getInstance().playSound("点击");
        XCT_JBT_DataManager.Instance.isShowingTakeoutPanel = false;
    ...
    onClickShowEvaluation(){
        XCT_AudioManager.getInstance().playSound("点击");
        this.EvaluationPanel.active = true; 
        this.EvaluationPanel.getChildByName('lblTime2').getComponent(Label).string = this.lbCurrentTime.string ;
        let btnBack = this.EvaluationPanel.getChildByName('btnBack');
        btnBack.off(Button.EventType.CLICK);
        btnBack.on(Button.EventType.CLICK, this.hideEvaluationPanel, this);

        let evaluationContainer = this.EvaluationPanel.getChildByName("ScrollView").getChildByName("view").getChildByName('evaluationContainer');
         // 清除现有订单
        for (let i = 0; i < evaluationContainer.children.length; i++) {
            if (i === 0) {
                evaluationContainer.children[i].active = false;
                continue;
            }
            evaluationContainer.children[i].destroy();
        }

        let takeoutItemPrefab = evaluationContainer.children[0];
        let evaluations = XCT_JBT_DataManager.Instance.playerData.historyEvaluation;
        // 创建新订单项
        evaluations.forEach(evaluation => {
            ...
            orderItem.active = true;
            // 设置头像(这里假设使用Sprite)
            orderItem.getChildByName('spAvatar').children.forEach((node: Node) => {
                if (node.name === evaluation.avatarName) {
                    node.active = true;
                }
                else {
                    node.active = false;
            ...
            let lblEvaluation = orderItem.getChildByName('lblEvaluation');
            lblEvaluation.getComponent(Label).string =XCT_JBT_DataManager.Instance.evaluationConfig[evaluation.avatarName][evaluation.star];

            let customerName = orderItem.getChildByName('lblName');
            customerName.getComponent(Label).string = evaluation.customerName;
            let price = orderItem.getChildByName("Layout").getChildByName('lblAdd');
            price.getComponent(Label).string = "收入：" + evaluation.price.toFixed(2);
            let refund = orderItem.getChildByName("Layout").getChildByName('lblRefund');
            refund.active = evaluation.refund > 0;
            refund.getComponent(Label).string = "-"+evaluation.refund.toFixed(2);
            orderItem.getChildByName('starContainer').children.forEach((node: Node) => {
                if (parseInt(node.name) <= evaluation.star) {
    ...
    hideEvaluationPanel(){
        this.EvaluationPanel.active = false;
    }
    ...
    addListener() {
        ...
        this.btnShowEvaluation.on(Button.EventType.CLICK, this.onClickShowEvaluation, this);
        // 添加数据更新事件监听(如果有)
        EventManager.on(XCT_Events.JBT_ORDER_DATA_UPDATED, this.refreshPanel, this);
        EventManager.on(XCT_Events.JBT_Update_Time, this.refreshPanel, this);
    }

    // 移除事件监听
    removeListener() {
        super.removeListener();
        EventManager.off(XCT_Events.JBT_ORDER_DATA_UPDATED, this.refreshPanel, this);
        EventManager.off(XCT_Events.JBT_Update_Time, this.refreshPanel, this);
