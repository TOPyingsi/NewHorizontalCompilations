...
@ccclass('XCT_KLM_GameUI')
export class XCT_KLM_GameUI extends XCT_BasePanel {
    ...
    updateUI() {
        this.lblDay.string = "第" + XCT_KLM_DataManager.Instance.playerData.currentDay + "天";
        this.updateTime();
        this.updateSmile();
        this.lblMoney.string = XCT_KLM_DataManager.Instance.playerData.money.toFixed(1);
    }
    ...
    updateTime() {
        ...
        spCom.fillRange = - (XCT_KLM_DataManager.Instance.currentTime + 8 * XCT_KLM_DataManager.Instance.MINUTES_PER_HOUR) / XCT_KLM_DataManager.Instance.totalTime;
    }

    updateSmile() {
        this.lblSmile.string = XCT_KLM_DataManager.Instance.currentSmile.toString() + "%";
    }
    ...
    showBackTipPanel() {
        XCT_AudioManager.getInstance().playSound("点击");
        XCT_UIManager.Instance.showPanel(XCT_UIPanel.KLM_TipPanel, null, () => {
            EventManager.Scene.emit(XCT_Events.KLM_ShowTip_Back);
        })
    }

    // 注册事件监听
    addListener() {
        EventManager.on(XCT_Events.KLM_Update_Money, this.updateMoney, this);
        EventManager.on(XCT_Events.KLM_Update_Time, this.updateTime, this);
        EventManager.on(XCT_Events.KLM_Update_Smile, this.updateSmile, this);
        EventManager.on(XCT_Events.KLM_Hide_smile, this.hideSmile, this);
        EventManager.on(XCT_Events.KLM_Show_smile, this.showSmile, this);
    ...
    // 注销事件监听
    removeListener() {
        EventManager.off(XCT_Events.KLM_Update_Money, this.updateMoney, this);
        EventManager.off(XCT_Events.KLM_Update_Time, this.updateTime, this);
        EventManager.off(XCT_Events.KLM_Update_Smile, this.updateSmile, this);
        EventManager.off(XCT_Events.KLM_Hide_smile, this.hideSmile, this);
        EventManager.off(XCT_Events.KLM_Show_smile, this.showSmile, this);
