...
@ccclass('XCT_KLM_DialoguePanel')
export class XCT_KLM_DialoguePanel extends XCT_BasePanel {
    ...
    init(isNeedShowTable: boolean = true) {
        ...
        if (XCT_KLM_DataManager.Instance.needShowTakeoutCome) {
            this.showTakeoutCom();
        }
        ...
        XCT_KLM_DataManager.Instance.isCatDriveAway = false;
        if (XCT_KLM_DataManager.Instance.playerData.currentDay == 1 || XCT_KLM_DataManager.Instance.playerData.isAdoptedCat) {
    ...
    onShowTableItem() {
        ...
        this.btnTakeout.active = XCT_KLM_DataManager.Instance.playerData.isTakeOutProjectSet;
        this.btnTakeout.getChildByName("nodeVideo").active = !XCT_KLM_DataManager.Instance.playerData.isTakeOutProjectOpen
    }
    ...
    onShowAddTip() {
        ...
        item.getComponentInChildren(Label).string = "小费+" + XCT_KLM_DataManager.Instance.currentSellingAddTip;
        // 设置初始位置和透明度
    ...
    onClickTakeout() {
        ...
        if (XCT_KLM_DataManager.Instance.playerData.isTakeOutProjectOpen) {
            ...
            this.btnTakeout.getChildByName("nodeOrderCome").active = false;
            XCT_UIManager.Instance.showPanel(XCT_UIPanel.KLM_TakeoutPanel);
        }
        else {
            XCT_UIManager.Instance.showPanel(XCT_UIPanel.KLM_TipPanel, null, () => {
                EventManager.Scene.emit(XCT_Events.KLM_ShowTip_OpenTakeout);
            })
        }
    ...
    showTakeoutCom() {
        if (XCT_KLM_DataManager.Instance.playerData.isTakeOutProjectOpen) {
            if (XCT_KLM_DataManager.Instance.isShowingTakeoutPanel) return;
    ...
    //         .call(() => {
    //             if (!XCT_KLM_DataManager.Instance.isTakeout) {
    ...
    onBtnDriveAwayClick() {
        XCT_KLM_DataManager.Instance.isCatDriveAway = true;
        let dialogue2 = this.btnCat.getChildByName("dialogue2")
    ...
    onBtnAdoptClick() {
        XCT_KLM_DataManager.Instance.playerData.isAdoptedCat = true;
        this.btnCat.active = false;
    }
        ...
        showDishNode() {
            this.cb = ()=>{
                ...
                tween(this.dishNode)
                    ...
                    .call(() => {
                        if(XCT_KLM_DataManager.Instance.isNeedShowDragTutorial){
        ...
        dishDragEnd() {
            if(XCT_KLM_DataManager.Instance.isNeedShowDragTutorial){
                ...
                this.dargTurtrialAnim.active = false;
                XCT_KLM_DataManager.Instance.isNeedShowDragTutorial = false;
            }
            if (XCT_KLM_DataManager.Instance.isTakeout) {
    
            }
            else {
                EventManager.Scene.emit(XCT_Events.KLM_Continue_Customer_EndDialog);
            }
    ...
    addListener() {
        ...
        EventManager.on(XCT_Events.KLM_Show_Add_Tip, this.onShowAddTip, this);

        // EventManager.on(XCT_Events.KLM_Show_Dish, this.showDish, this);
        // EventManager.on(XCT_Events.KLM_Take_Dish, this.takeDish, this);

        EventManager.on(XCT_Events.KLM_Takeout_Open, this.takeoutOpen, this);
        EventManager.on(XCT_Events.KLM_Takeout_Open, this.takeoutOpen, this);

        EventManager.on(XCT_Events.KLM_New_Takeout_Order, this.showTakeoutCom, this);
        EventManager.on(XCT_Events.KLM_Hide_New_Takeout_Order, this.hideTakeoutCom, this);

        EventManager.on(XCT_Events.KLM_Show_Dish_Node, this.showDishNode, this);
    ...
    removeListener() {
        ...
        EventManager.off(XCT_Events.KLM_Show_Add_Tip, this.onShowAddTip, this);
        // EventManager.off(XCT_Events.KLM_Show_Dish, this.showDish, this);
        // EventManager.off(XCT_Events.KLM_Take_Dish, this.takeDish, this);
        EventManager.off(XCT_Events.KLM_New_Takeout_Order, this.showTakeoutCom, this);
        EventManager.off(XCT_Events.KLM_Hide_New_Takeout_Order, this.hideTakeoutCom, this);
        EventManager.off(XCT_Events.KLM_Takeout_Open, this.takeoutOpen, this);
         EventManager.off(XCT_Events.KLM_Show_Dish_Node, this.showDishNode, this);
