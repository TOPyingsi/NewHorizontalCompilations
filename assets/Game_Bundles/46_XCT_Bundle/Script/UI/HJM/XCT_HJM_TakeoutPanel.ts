...
import { XCT_HJM_OrderStatus, XCT_HJM_TakeoutOrder, XCT_HJM_DataManager } from '../../Manager/XCT_HJM_DataManager';
...
@ccclass('XCT_HJM_TakeoutPanel')
export class XCT_HJM_TakeoutPanel extends XCT_BasePanel {
    ...
    init() {
        ...
        XCT_HJM_DataManager.Instance.isShowingTakeoutPanel = true;
        super.init();
        this.refreshPanel();
    ...
    refreshPanel() {
        // 获取店铺信息并更新
        const hour = XCT_HJM_DataManager.Instance.currentHour.toString().padStart(2, '0');
        const minute = XCT_HJM_DataManager.Instance.currentMinute.toString().padStart(2, '0');
        this.lbCurrentTime.string = `${hour}:${minute}`;
        this.updateStars(XCT_HJM_DataManager.Instance.playerData.score);

        // 获取订单列表并刷新
        const orders: XCT_HJM_TakeoutOrder[] = XCT_HJM_DataManager.Instance.currentTakeoutOrders;
        this.nodeNoOrder.active = orders.length === 0;
        //将orders里的内容倒序排列到一个新数组里，再刷新列表
        const newOrders: XCT_HJM_TakeoutOrder[] = [];
    ...
    // 刷新订单列表
    refreshOrderList(orders: XCT_HJM_TakeoutOrder[]) {
        // 清除现有订单
        ...
        orders.forEach(order => {
           ...
           XCT_HJM_DataManager.Instance.lastTakeoutOrderIds = XCT_HJM_DataManager.Instance.currentTakeoutOrders.map(order => order.id);
    }

    // 更新单个订单项
    updateOrderItem(itemNode: Node, order: XCT_HJM_TakeoutOrder) {
        let lastTakeoutOrderIds = XCT_HJM_DataManager.Instance.lastTakeoutOrderIds;
        ...
        switch (order.status) {
            case XCT_HJM_OrderStatus.UNHANDLED:
                if (order.missingStockIngredients.length > 0) {
                ...
                break;
            case XCT_HJM_OrderStatus.TIMEOUT:
                nodeTimeout.active = true;
                break;
            case XCT_HJM_OrderStatus.COOKED:
                nodeCookEnd.active = true;
                break;
            case XCT_HJM_OrderStatus.DELIVERED:
                nodeDelivered.active = true;
                break;
            case XCT_HJM_OrderStatus.REFUSED:
    ...
    // 处理接单
    handleVideo(orderData: XCT_HJM_TakeoutOrder, missLabelNode: Node) {
        XCT_AudioManager.getInstance().playSound("点击");
        Banner.Instance.ShowVideoAd(() => {

            // 假设视频播放完成
            if (XCT_HJM_DataManager.Instance.playerData.stock[orderData.missingStockIngredients[0]]) {
                XCT_HJM_DataManager.Instance.playerData.stock[orderData.missingStockIngredients[0]]++;
            } else {
                XCT_HJM_DataManager.Instance.playerData.stock[orderData.missingStockIngredients[0]] = 1;
            }
        ...
        // XCT_HJM_DataManager.Instance.updateOrderStatus(orderId, OrderStatus.COOKED);
    }

    // 处理接单
    handleAcceptOrder(orderData: XCT_HJM_TakeoutOrder) {
        XCT_AudioManager.getInstance().playSound("点击");

        XCT_HJM_DataManager.Instance.resetTakout(orderData);
        XCT_HJM_DataManager.Instance.takeoutNotes.push([orderData.content, `备注: ${orderData.remark}`])
    ...
    // 处理拒单
    handleRefuseOrder(orderData: XCT_HJM_TakeoutOrder) {
        XCT_AudioManager.getInstance().playSound("点击");
        // 实际项目中可能需要确认逻辑
        orderData.status = XCT_HJM_OrderStatus.REFUSED;
        // XCT_HJM_DataManager.Instance.updateOrderStatus(orderId, OrderStatus.TIMEOUT);
        this.refreshPanel(); // 刷新面板
    }

    onClickReturn() {
        XCT_AudioManager.getInstance().playSound("点击");
        XCT_HJM_DataManager.Instance.isShowingTakeoutPanel = false;
        XCT_UIManager.Instance.hidePanel(XCT_UIPanel.HJM_TakeoutPanel);
    }
    ...
    addListener() {
        ...
        // 添加数据更新事件监听(如果有)
        EventManager.on(XCT_Events.HJM_ORDER_DATA_UPDATED, this.refreshPanel, this);
        EventManager.on(XCT_Events.HJM_Update_Time, this.refreshPanel, this);
    }

    // 移除事件监听
    removeListener() {
        super.removeListener();
        EventManager.off(XCT_Events.HJM_ORDER_DATA_UPDATED, this.refreshPanel, this);
        EventManager.off(XCT_Events.HJM_Update_Time, this.refreshPanel, this);
    }
