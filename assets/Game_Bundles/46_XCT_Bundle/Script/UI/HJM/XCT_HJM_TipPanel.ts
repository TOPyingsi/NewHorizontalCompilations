...
@ccclass('XCT_HJM_TipPanel')
export class XCT_HJM_TipPanel extends XCT_BasePanel {
  ...
  onBtnExit() {
    ...
    if(XCT_GameManager.Instance.hasStartPanel){
          XCT_UIManager.Instance.hidePanel(XCT_UIPanel.HJM_TipPanel);
          ...
          XCT_UIManager.Instance.showPanel(XCT_UIPanel.TablePanel, null, () => {
            XCT_UIManager.Instance.showPanel(XCT_UIPanel.LoadingPanel, () => {
              XCT_UIManager.Instance.showPanel(XCT_UIPanel.StartPanel, null, () => {
                ...
                XCT_UIManager.Instance.hidePanel(XCT_UIPanel.HJM_TutorialPanel);
                XCT_UIManager.Instance.hidePanel(XCT_UIPanel.HJM_GamePanel, null, XCT_PanelAnimation.NONE);
  ...
  hideNoMoneyTipPanel() {
     if(!XCT_HJM_DataManager.Instance.isShowingShopPanel){
            EventManager.Scene.emit(XCT_Events.Game_Resume);
        }
    XCT_AudioManager.getInstance().playSound("点击");
    XCT_UIManager.Instance.hidePanel(XCT_UIPanel.HJM_TipPanel);
  ...
  btnVideoClick() {
    ...
    Banner.Instance.ShowVideoAd(() => {
      // 假设视频播放完成
      XCT_HJM_DataManager.Instance.playerData.money += 200;
  ...
  btnVideoOpenTakeoutClick() {
    ...
    // 这里应该调用视频广告接口
    Banner.Instance.ShowVideoAd(() => {
      XCT_HJM_DataManager.Instance.playerData.isTakeOutProjectOpen = true;
      EventManager.Scene.emit(XCT_Events.HJM_Takeout_Open);
      // 假设视频播放完成
      XCT_UIManager.Instance.showPanel(XCT_UIPanel.HJM_TakeoutPanel, null, () => {
        XCT_UIManager.Instance.hidePanel(XCT_UIPanel.HJM_TipPanel);
      });
    });
  }
  ...
  addListener() {
    ...
    EventManager.on(XCT_Events.HJM_ShowTip_Back, this.showBackTipPanel, this);
    EventManager.on(XCT_Events.HJM_ShowTip_NoMoney, this.showNoMoneyTipPanel, this);

    EventManager.on(XCT_Events.HJM_ShowTip_OpenTakeout, this.showOpenTakeoutPanel, this);
  ...
  removeListener() {
    ...
    EventManager.off(XCT_Events.HJM_ShowTip_Back, this.showBackTipPanel, this);
    EventManager.off(XCT_Events.HJM_ShowTip_NoMoney, this.showNoMoneyTipPanel, this);
    EventManager.off(XCT_Events.HJM_ShowTip_OpenTakeout, this.showOpenTakeoutPanel, this);
