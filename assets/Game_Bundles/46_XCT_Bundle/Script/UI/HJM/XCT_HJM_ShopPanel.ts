...
@ccclass('XCT_HJM_ShopPanel')
export class XCT_HJM_ShopPanel extends XCT_BasePanel {
    ...
    init(isCooking: boolean = false) {
         ProjectEventManager.emit(ProjectEvent.弹出窗口, "小吃摊");
                        XCT_HJM_DataManager.Instance.isShowingShopPanel = true;
    ...
    initShopItem() {
        ...
        XCT_HJM_DataManager.Instance.ingredientsConfig.forEach((shopConfig, index) => {
            let ingredientName = shopConfig.name;
            const hasIngredient = XCT_HJM_DataManager.Instance.playerData.stock[ingredientName] !== undefined && XCT_HJM_DataManager.Instance.playerData.stock[ingredientName] > 0; // 假设库存检查结果
            if (!hasIngredient) {
                ...
                let color = XCT_HJM_DataManager.Instance.playerData.money >= shopConfig.price ? Color.WHITE : Color.RED;
    ...
    updateShopItem() {
        this.shopItemContainer.children.forEach((shopItem) => {
            let color = XCT_HJM_DataManager.Instance.playerData.money >= this.currentData.get(shopItem.name) ? Color.WHITE : Color.RED;
            shopItem.getChildByName('btnBuy').getChildByName('lblPrice').getComponent(Label).color = color;
        })
    }
    ...
    onClickBuy(ingredientName: string) {
        ...
        if (XCT_HJM_DataManager.Instance.playerData.money < XCT_HJM_DataManager.Instance.ingredientsConfigObject[ingredientName].price) {
            XCT_UIManager.Instance.showPanel(XCT_UIPanel.HJM_TipPanel, null, () => {
                EventManager.Scene.emit(XCT_Events.HJM_ShowTip_NoMoney);
            })

            return;
        }
        // 进货
        if (XCT_HJM_DataManager.Instance.playerData.stock[ingredientName]) {
            XCT_HJM_DataManager.Instance.playerData.stock[ingredientName]++;
        } else {
            XCT_HJM_DataManager.Instance.playerData.stock[ingredientName] = 1;
        }
        XCT_HJM_DataManager.Instance.playerData.money -= XCT_HJM_DataManager.Instance.ingredientsConfigObject[ingredientName].price;
        EventManager.Scene.emit(XCT_Events.HJM_Update_Money);
        EventManager.Scene.emit(XCT_Events.HJM_Update_Ingredient);
    ...
    onClickCheckout() {
        ...
        // XCT_UIManager.Instance.hidePanel(XCT_UIPanel.HJM_ShopPanel);
    }

    onClickNextDay() {
        XCT_AudioManager.getInstance().playSound("点击");
        XCT_UIManager.Instance.showPanel(XCT_UIPanel.PassPanel, {
            tipConfig: XCT_HJM_DataManager.Instance.tipConfig,
            currentDay: XCT_HJM_DataManager.Instance.playerData.currentDay
            , passDayCb: () => {
                XCT_HJM_DataManager.Instance.passDay();
            }
        }, () => {
            XCT_UIManager.Instance.hidePanel(XCT_UIPanel.HJM_ShopPanel);
            XCT_UIManager.Instance.hidePanel(XCT_UIPanel.HJM_GameUI);
        });

        XCT_HJM_DataManager.Instance.isHadShowShop = false;
    }
    ...
    onClickReturn() {
        ...
        XCT_HJM_DataManager.Instance.isShowingShopPanel = false;
    }
    ...
    // 注册事件监听
    addListener() {
        EventManager.on(XCT_Events.HJM_Update_Money, this.updateShopItem, this)
    ...
    // 注销事件监听
    removeListener() {
        EventManager.off(XCT_Events.HJM_Update_Money, this.updateShopItem, this)
