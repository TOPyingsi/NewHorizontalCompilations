...
@ccclass('XCT_LSF_DialoguePanel')
export class XCT_LSF_DialoguePanel extends XCT_BasePanel {
    ...
    init(isNeedShowTable: boolean = true) {
        ...
        if (XCT_LSF_DataManager.Instance.needShowTakeoutCome) {
            this.showTakeoutCom();
        }
        ...
        XCT_LSF_DataManager.Instance.isCatDriveAway = false;
        if (XCT_LSF_DataManager.Instance.playerData.currentDay == 1 || XCT_LSF_DataManager.Instance.playerData.isAdoptedCat) {
    ...
    onShowTableItem() {
        ...
        this.btnTakeout.active = XCT_LSF_DataManager.Instance.playerData.isTakeOutProjectSet;
        this.btnTakeout.getChildByName("nodeVideo").active = !XCT_LSF_DataManager.Instance.playerData.isTakeOutProjectOpen
    }
    ...
    onShowAddTip() {
        ...
        item.getComponentInChildren(Label).string = "小费+" + XCT_LSF_DataManager.Instance.currentSellingAddTip;
        // 设置初始位置和透明度
    ...
    onClickTakeout() {
        ...
        if (XCT_LSF_DataManager.Instance.playerData.isTakeOutProjectOpen) {
            ...
            this.btnTakeout.getChildByName("nodeOrderCome").active = false;
            XCT_UIManager.Instance.showPanel(XCT_UIPanel.LSF_TakeoutPanel);
        }
        else {
            XCT_UIManager.Instance.showPanel(XCT_UIPanel.LSF_TipPanel, null, () => {
                EventManager.Scene.emit(XCT_Events.LSF_ShowTip_OpenTakeout);
            })
        }
    ...
    showTakeoutCom() {
        if (XCT_LSF_DataManager.Instance.playerData.isTakeOutProjectOpen) {
            if (XCT_LSF_DataManager.Instance.isShowingTakeoutPanel) return;
    ...
    //         .call(() => {
    //             if (!XCT_LSF_DataManager.Instance.isTakeout) {
    ...
    onBtnDriveAwayClick() {
        XCT_LSF_DataManager.Instance.isCatDriveAway = true;
        let dialogue2 = this.btnCat.getChildByName("dialogue2")
    ...
    onBtnAdoptClick() {
        XCT_LSF_DataManager.Instance.playerData.isAdoptedCat = true;
        this.btnCat.active = false;
    }
            ...
            showDishNode() {
                this.cb = ()=>{
                    ...
                    tween(this.dishNode)
                        ...
                        .call(() => {
                            if(XCT_LSF_DataManager.Instance.isNeedShowDragTutorial){
            ...
            dishDragEnd() {
                if(XCT_LSF_DataManager.Instance.isNeedShowDragTutorial){
                    ...
                    this.dargTurtrialAnim.active = false;
                    XCT_LSF_DataManager.Instance.isNeedShowDragTutorial = false;
                }
                if (XCT_LSF_DataManager.Instance.isTakeout) {
        
                }
                else {
                    EventManager.Scene.emit(XCT_Events.LSF_Continue_Customer_EndDialog);
                }
    ...
    addListener() {
        ...
        EventManager.on(XCT_Events.LSF_Show_Add_Tip, this.onShowAddTip, this);

        // EventManager.on(XCT_Events.LSF_Show_Dish, this.showDish, this);
        // EventManager.on(XCT_Events.LSF_Take_Dish, this.takeDish, this);

        EventManager.on(XCT_Events.LSF_Takeout_Open, this.takeoutOpen, this);

        EventManager.on(XCT_Events.LSF_New_Takeout_Order, this.showTakeoutCom, this);
        EventManager.on(XCT_Events.LSF_Hide_New_Takeout_Order, this.hideTakeoutCom, this);
        
        EventManager.on(XCT_Events.LSF_Show_Dish_Node, this.showDishNode, this);
        EventManager.on(XCT_Events.Dish_Drag_End, this.dishDragEnd, this);

        EventManager.on(XCT_Events.LSF_Show_Bowl, this.showBowl, this);
    ...
    removeListener() {
        ...
        EventManager.off(XCT_Events.LSF_Show_Add_Tip, this.onShowAddTip, this);
        // EventManager.off(XCT_Events.LSF_Show_Dish, this.showDish, this);
        // EventManager.off(XCT_Events.LSF_Take_Dish, this.takeDish, this);
        EventManager.off(XCT_Events.LSF_New_Takeout_Order, this.showTakeoutCom, this);
        EventManager.off(XCT_Events.LSF_Hide_New_Takeout_Order, this.hideTakeoutCom, this);
        EventManager.off(XCT_Events.LSF_Takeout_Open, this.takeoutOpen, this);

        
        EventManager.off(XCT_Events.LSF_Show_Dish_Node, this.showDishNode, this);
        EventManager.off(XCT_Events.Dish_Drag_End, this.dishDragEnd, this);
        EventManager.off(XCT_Events.LSF_Show_Bowl, this.showBowl, this);
