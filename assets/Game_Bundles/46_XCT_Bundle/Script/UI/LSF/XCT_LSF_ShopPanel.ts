...
@ccclass('XCT_LSF_ShopPanel')
export class XCT_LSF_ShopPanel extends XCT_BasePanel {
    ...
    init(isCooking: boolean = false) {
         ProjectEventManager.emit(ProjectEvent.弹出窗口, "小吃摊");
                        XCT_LSF_DataManager.Instance.isShowingShopPanel = true;
    ...
    initShopItem() {
        ...
        XCT_LSF_DataManager.Instance.ingredientsConfig.forEach((shopConfig, index) => {
            let ingredientName = shopConfig.name;
            const hasIngredient = XCT_LSF_DataManager.Instance.playerData.stock[ingredientName] !== undefined && XCT_LSF_DataManager.Instance.playerData.stock[ingredientName] > 0; // 假设库存检查结果
            if (!hasIngredient) {
                ...
                if (ingredientName === "鸡精") {
                    shopItem.getChildByName('icon').setPosition(shopItem.getChildByName('icon').position.x + 50, shopItem.getChildByName('icon').position.y);
                }
                ...
                let color = XCT_LSF_DataManager.Instance.playerData.money >= shopConfig.price ? Color.WHITE : Color.RED;
    ...
    onClickBuy(ingredientName: string) {
        ...
        if (XCT_LSF_DataManager.Instance.playerData.money < XCT_LSF_DataManager.Instance.ingredientsConfigObject[ingredientName].price) {
        ...
        // 进货
        if (XCT_LSF_DataManager.Instance.playerData.stock[ingredientName]) {
            XCT_LSF_DataManager.Instance.playerData.stock[ingredientName]++;
        } else {
            XCT_LSF_DataManager.Instance.playerData.stock[ingredientName] = 1;
        }
        XCT_LSF_DataManager.Instance.playerData.money -= XCT_LSF_DataManager.Instance.ingredientsConfigObject[ingredientName].price;
        EventManager.Scene.emit(XCT_Events.LSF_Update_Money);
        EventManager.Scene.emit(XCT_Events.LSF_Update_Ingredient);
    ...
    updateShopItem() {
        this.shopItemContainer.children.forEach((shopItem) => {
            let color = XCT_LSF_DataManager.Instance.playerData.money >= this.currentData.get(shopItem.name) ? Color.WHITE : Color.RED;
            shopItem.getChildByName('btnBuy').getChildByName('lblPrice').getComponent(Label).color = color;
        })
    }
    ...
    onClickCheckout() {
        ...
        // XCT_UIManager.Instance.hidePanel(XCT_UIPanel.LSF_ShopPanel);
    }
    ...
    onClickNextDay() {
        ...
        XCT_UIManager.Instance.showPanel(XCT_UIPanel.PassPanel, {
            tipConfig: XCT_LSF_DataManager.Instance.tipConfig,
            currentDay: XCT_LSF_DataManager.Instance.playerData.currentDay
            , passDayCb: () => {
                XCT_LSF_DataManager.Instance.passDay();
            }
        }, () => {
            XCT_UIManager.Instance.hidePanel(XCT_UIPanel.LSF_ShopPanel);
            XCT_UIManager.Instance.hidePanel(XCT_UIPanel.LSF_GameUI);
        });

        XCT_LSF_DataManager.Instance.isHadShowShop = false;
    }
    ...
    onClickReturn() {
        ...
        XCT_LSF_DataManager.Instance.isShowingShopPanel = false;
    }
    ...
    // 注册事件监听
    addListener() {
        EventManager.on(XCT_Events.LSF_Update_Money, this.updateShopItem, this)
    ...
    // 注销事件监听
    removeListener() {
        EventManager.off(XCT_Events.LSF_Update_Money, this.updateShopItem, this)
