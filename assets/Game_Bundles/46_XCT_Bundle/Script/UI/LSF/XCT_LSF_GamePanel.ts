...
@ccclass('XCT_LSF_GamePanel')
export class XCT_LSF_GamePanel extends XCT_BasePanel {
    ...
    private onCompleted(): void {
        ...
        XCT_LSF_DataManager.Instance.endCooking(bowl);
    }
    ...
    updateIngredientContainer() {
        ...
        this.ingredientContainer.children.forEach((node: Node) => {
            if (XCT_LSF_DataManager.Instance.playerData.stock[node.name] && XCT_LSF_DataManager.Instance.playerData.stock[node.name] > 0) {
                node.active = true;
                count++;
    ...
    onClickEmptyIngredient() {
        ...
        XCT_UIManager.Instance.showPanel(XCT_UIPanel.LSF_ShopPanel, true);
    }
    ...
    onClickNote() {
        ...
        nodeNode1.active = !XCT_LSF_DataManager.Instance.isTakeout;
        nodeNode2.active = XCT_LSF_DataManager.Instance.isTakeout;

        let noteNodeName = XCT_LSF_DataManager.Instance.isTakeout ? "takeoutNoteNode" : "noteNode";
        let node = this.NotePanel.getChildByName(noteNodeName)
        let noteContainer = node.getChildByName("noteContainer");
        let notes = XCT_LSF_DataManager.Instance.isTakeout ? XCT_LSF_DataManager.Instance.takeoutNotes : XCT_LSF_DataManager.Instance.notes;
    ...
    update(dt: number) {
        if (XCT_LSF_DataManager.Instance.isPause ||
            !this.isArea2 ||
            XCT_LSF_DataManager.Instance.playerData.isAdoptedCat ||
            XCT_LSF_DataManager.Instance.isCatDriveAway ||
            XCT_LSF_DataManager.Instance.playerData.currentDay == 1 ||
            !XCT_LSF_DataManager.Instance.isCooking) return;
    ...
    catSteal(paw: Node, idx: number) {
        ...
        this.tween = tween(pawSp)
            ...
            .call(() => {
                ...
                let cost = XCT_LSF_DataManager.Instance.ingredientsConfigObject[stealIngredient].cost;
                XCT_LSF_DataManager.Instance.playerData.money -= cost;
    ...
    // 注册事件监听
    addListener() {
        EventManager.on(XCT_Events.LSF_Update_Ingredient, this.updateIngredientContainer, this);

        EventManager.on(XCT_Events.LSF_Change_CookArea, this.changeCookArea, this);
        // EventManager.on(XCT_Events.LSF_Pack_Ingredients, this.onPackIngredients, this);
        EventManager.on(XCT_Events.LSF_Completed, this.onCompleted, this);
    ...
    // 注销事件监听
    removeListener() {
        EventManager.off(XCT_Events.LSF_Update_Ingredient, this.updateIngredientContainer, this);
        EventManager.off(XCT_Events.LSF_Change_CookArea, this.changeCookArea, this);
        // EventManager.off(XCT_Events.LSF_Pack_Ingredients, this.onPackIngredients, this);
        EventManager.off(XCT_Events.LSF_Completed, this.onCompleted, this);
