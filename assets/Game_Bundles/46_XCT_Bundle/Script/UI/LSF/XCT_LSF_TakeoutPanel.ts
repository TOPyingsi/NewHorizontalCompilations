...
import { XCT_LSF_OrderStatus, XCT_LSF_TakeoutOrder, XCT_LSF_DataManager } from '../../Manager/XCT_LSF_DataManager';
...
@ccclass('XCT_LSF_TakeoutPanel')
export class XCT_LSF_TakeoutPanel extends XCT_BasePanel {
    ...
    init() {
        ...
        XCT_LSF_DataManager.Instance.isShowingTakeoutPanel = true;
        super.init();
        this.refreshPanel();
    ...
    refreshPanel() {
        ...
        this.updateStars(XCT_LSF_DataManager.Instance.playerData.score);

        // 获取订单列表并刷新
        const orders: XCT_LSF_TakeoutOrder[] = XCT_LSF_DataManager.Instance.currentTakeoutOrders;
        this.nodeNoOrder.active = orders.length === 0;
        const newOrders: XCT_LSF_TakeoutOrder[] = [];
    ...
    // 刷新订单列表
    refreshOrderList(orders: XCT_LSF_TakeoutOrder[]) {
        // 清除现有订单
        ...
        orders.forEach(order => {
           ...
           XCT_LSF_DataManager.Instance.lastTakeoutOrderIds = XCT_LSF_DataManager.Instance.currentTakeoutOrders.map(order => order.id);
    }

    // 更新单个订单项
    updateOrderItem(itemNode: Node, order: XCT_LSF_TakeoutOrder) {
        let lastTakeoutOrderIds = XCT_LSF_DataManager.Instance.lastTakeoutOrderIds;
        ...
        switch (order.status) {
            case XCT_LSF_OrderStatus.UNHANDLED:
                if (order.missingStockIngredients.length > 0) {
                ...
                break;
            case XCT_LSF_OrderStatus.TIMEOUT:
                nodeTimeout.active = true;
                break;
            case XCT_LSF_OrderStatus.COOKED:
                nodeCookEnd.active = true;
                break;
            case XCT_LSF_OrderStatus.DELIVERED:
                nodeDelivered.active = true;
                break;
            case XCT_LSF_OrderStatus.REFUSED:
    ...
    // 处理接单
    handleVideo(orderData: XCT_LSF_TakeoutOrder, missLabelNode: Node) {
        XCT_AudioManager.getInstance().playSound("点击");
        Banner.Instance.ShowVideoAd(() => {
            // 假设视频播放完成
            if (XCT_LSF_DataManager.Instance.playerData.stock[orderData.missingStockIngredients[0]]) {
                XCT_LSF_DataManager.Instance.playerData.stock[orderData.missingStockIngredients[0]]++;
            } else {
                XCT_LSF_DataManager.Instance.playerData.stock[orderData.missingStockIngredients[0]] = 1;
            }
        ...
        // XCT_LSF_DataManager.Instance.updateOrderStatus(orderId, OrderStatus.COOKED);
    }

    // 处理接单
    handleAcceptOrder(orderData: XCT_LSF_TakeoutOrder) {
        XCT_AudioManager.getInstance().playSound("点击");
XCT_LSF_DataManager.Instance.resetTakout(orderData);
           XCT_LSF_DataManager.Instance.takeoutNotes.push([orderData.content, `备注: ${orderData.remark}`])
    ...
    // 处理拒单
    handleRefuseOrder(orderData: XCT_LSF_TakeoutOrder) {
        XCT_AudioManager.getInstance().playSound("点击");
        // 实际项目中可能需要确认逻辑
        orderData.status = XCT_LSF_OrderStatus.REFUSED;
        // XCT_LSF_DataManager.Instance.updateOrderStatus(orderId, OrderStatus.TIMEOUT);
        this.refreshPanel(); // 刷新面板
    }

    onClickReturn() {
        XCT_AudioManager.getInstance().playSound("点击");
        XCT_LSF_DataManager.Instance.isShowingTakeoutPanel = false;
        XCT_UIManager.Instance.hidePanel(XCT_UIPanel.LSF_TakeoutPanel);
    }
  ...
  onHideComplete() {
    ...
    addListener() {
        ...
        // 添加数据更新事件监听(如果有)
        EventManager.on(XCT_Events.LSF_ORDER_DATA_UPDATED, this.refreshPanel, this);
        EventManager.on(XCT_Events.LSF_Update_Time, this.refreshPanel, this);
    }

    // 移除事件监听
    removeListener() {
        EventManager.off(XCT_Events.LSF_ORDER_DATA_UPDATED, this.refreshPanel, this);
        EventManager.off(XCT_Events.LSF_Update_Time, this.refreshPanel, this);
    }
