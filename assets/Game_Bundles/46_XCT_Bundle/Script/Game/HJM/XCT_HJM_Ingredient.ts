...
@ccclass('XCT_HJM_Ingredient')
export class XCT_HJM_Ingredient extends Component {
    ...
    private onTouchStart(event: Touch): void {
        ...
        if (XCT_HJM_DataManager.Instance.tutorialIdx == 10 && this.node.name == "洋葱") {
            EventManager.Scene.emit(XCT_Events.HandAnimation_End);
            XCT_HJM_DataManager.Instance.tutorialIdx++;
        }

        if (XCT_HJM_DataManager.Instance.tutorialIdx == 12 && this.node.name == "咸鸭蛋") {
            EventManager.Scene.emit(XCT_Events.HandAnimation_End);
            XCT_HJM_DataManager.Instance.tutorialIdx++;
        }
    ...
    private onClick() {
        ...
        if(XCT_HJM_DataManager.Instance.playerData.currentDay == 1 &&!XCT_HJM_DataManager.Instance.isTutorialEnd){
            ...
            if (XCT_HJM_DataManager.Instance.tutorialIdx == 9 && this.node.name !== "洋葱") {
                return;
            }
            if (XCT_HJM_DataManager.Instance.tutorialIdx == 11 && this.node.name !== "咸鸭蛋") {
                return;
            }
        }
