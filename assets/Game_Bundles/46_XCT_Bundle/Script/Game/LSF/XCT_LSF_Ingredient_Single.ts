...
@ccclass('XCT_LSF_Ingredient_Single')
export class XCT_LSF_Ingredient_Single extends Component {
    ...
    private onTouchStart(event: Touch): void {
        if (XCT_LSF_DataManager.Instance.tutorialIdx == 1 && this.node.name == "玉米排骨锅") {
            EventManager.Scene.emit(XCT_Events.HandAnimation_End);
            XCT_LSF_DataManager.Instance.tutorialIdx++;
        }
        if (XCT_LSF_DataManager.Instance.tutorialIdx == 3 && this.node.name == "宽粉") {
            EventManager.Scene.emit(XCT_Events.HandAnimation_End);
            XCT_LSF_DataManager.Instance.tutorialIdx++;
        }
    }
    ...
    private onClick() {
        ...
        let noodles = ["碱面", "方便面", "宽粉", "拉面", "细粉"]
        if (this.IngredientType == 1) {
            let hasSoup = false;

            XCT_LSF_DataManager.Instance.currentCookedSteps.forEach((soup: string) => {
                if (hasSoup) return;
            ...
            XCT_LSF_DataManager.Instance.currentCookedSteps.forEach((noodle: string) => {
                if (hasNoodle) return;
        ...
        if(XCT_LSF_DataManager.Instance.playerData.currentDay == 1 &&!XCT_LSF_DataManager.Instance.isTutorialEnd){
            ...
            if (XCT_LSF_DataManager.Instance.tutorialIdx == 0 && this.node.name !== "玉米排骨锅") {
                return;
            }
            if (XCT_LSF_DataManager.Instance.tutorialIdx == 2 && this.node.name !== "宽粉") {
                return;
            }
        }
