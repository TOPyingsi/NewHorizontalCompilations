...
@ccclass('XCT_LSF_Salt')
export class XCT_LSF_Salt extends Component {
    ...
    private onClick() {
        ...
        if(XCT_LSF_DataManager.Instance.playerData.currentDay == 1 &&!XCT_LSF_DataManager.Instance.isTutorialEnd){
            ...
            if (XCT_LSF_DataManager.Instance.tutorialIdx == 7 && this.node.name !== "盐") {
                return;
            }
        }
    ...
    onSauceNodeClick() {
        ...
        if (XCT_LSF_DataManager.Instance.tutorialIdx == 8 && this.node.name == "盐") {
            EventManager.Scene.emit(XCT_Events.HandAnimation_End);
            XCT_LSF_DataManager.Instance.tutorialIdx++;
        }
