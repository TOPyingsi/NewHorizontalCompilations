...
// 配料类型枚举
export enum XCT_LSF_IngredientType {
    SPICE = "撒料类",
    SPREAD = "涂抹类",
    EGG = "鸡蛋类"
}
...
// 按钮回调类型枚举
export enum XCT_LSF_ButtonAction {
    START_MAKE = "开始制作",
    NEXT_DIALOG = "下一段对话",
    BUY_INGREDIENT = "购买食材",
    REFUSE = "拒绝",
...
// 配料要求接口
export interface XCT_LSF_IngredientRequire {
    type: XCT_LSF_IngredientType;
    need: 0 | 1;
    count?: number;
    percentage?: number;
    rotateCount?: number;
...
// 订单接口
export interface XCT_LSF_Order {
    [key: string]: XCT_LSF_IngredientRequire; // 配料名: 要求
}

// 对话段落接口
export interface XCT_LSF_DialogSegment {
    id: number;
    buttons: XCT_LSF_DialogButton[];
    checkStock: boolean;
    text: string;
}

// 对话按钮接口
export interface XCT_LSF_DialogButton {
    text: string;
    action: XCT_LSF_ButtonAction;
    nextSegment: number; // -1:无 0:默认++ 其他:指定编号
}

// 订单检查结果接口
export interface XCT_LSF_OrderCheckResult {
...
// 客人接口
export interface XCT_LSF_Customer {
    id: number;
    avatar: string;
    order: XCT_LSF_Order;
    startDialogs: XCT_LSF_DialogSegment[];
    endDialogs: XCT_LSF_DialogSegment[];
}

// 天数配置接口
export interface XCT_LSF_DayConfig {
    day: number;
    customers: XCT_LSF_Customer[];
}


export interface XCT_LSF_PlayerData {
    money: number;
    currentDay: number;
...
export interface XCT_LSF_IngredientsConfig {
    name: string;
    price: number;
    cost: number,
    sellingPrice: number
...
export enum XCT_LSF_OrderStatus {
    UNHANDLED = 0,    // 未处理
    TIMEOUT = 1,      // 已超时
    COOKED = 2,       // 制作完成
    DELIVERED = 3,     // 已送达
...
// 单个订单数据接口
export interface XCT_LSF_TakeoutOrderConfig {
    id: string;               // 订单唯一标识
    ...
    content: string;          // 订单内容
    remark: string;           // 备注信息
    order: XCT_LSF_Order;            // 订单配置
}

// 外卖订单天数配置接口
export interface XCT_LSF_DayTakeoutConfig {
    day: number;
    takeoutOrders: XCT_LSF_TakeoutOrderConfig[];
}

// 单个订单数据接口
export interface XCT_LSF_TakeoutOrder {
    id: number;               // 订单唯一标识
    avatarName: string;        // 头像资源路径
    customerName: string;     // 点单人昵称
    expireTime: number[];       // 过期时间(如[14,00])
    ...
    price: number;            // 订单价格
    order: XCT_LSF_Order;            // 订单配置
    status: XCT_LSF_OrderStatus;      // 订单状态
    missingStockIngredients: string[];
}

// 数据管理单例
@ccclass('XCT_LSF_DataManager')
export class XCT_LSF_DataManager extends Component {
    public static Instance: XCT_LSF_DataManager;
    private STORAGE_KEY = "XCT_LSF_PlayerData"
    ...
    // 商店配置
    public ingredientsConfig: XCT_LSF_IngredientsConfig[] = null!;
    public ingredientsConfigObject: { [key: string]: XCT_LSF_IngredientsConfig } = null!;

    public tipConfig: { [key: string]: string } = null!;

    // 外卖订单配置
    public takeoutOrderConfig: XCT_LSF_DayTakeoutConfig[] = null!;

    public standardSpreadCount: number = 300;


    // 游戏数据
    public playerData: XCT_LSF_PlayerData = null;
    ...
    public currentCustomer: XCT_LSF_Customer = null;
    ...
    // 外卖订单相关
    public currentDayTakeoutConfig: XCT_LSF_TakeoutOrderConfig[] = [];
    public currentTakeoutOrders: XCT_LSF_TakeoutOrder[] = []; // 当前处理的订单列表
        public lastTakeoutOrderIds: number[] = []; // 最近处理的订单ID列表
    public currentTakeoutOrder: XCT_LSF_TakeoutOrder = null; // 当前处理的订单
    public needShowTakeoutCome: boolean = false; // 是否需要展示外卖订单结果

    public notes: string[][] = []; // 订单笔记
    public takeoutNotes: string[][] = []; // 外卖笔记

    //制作结果相关
    public currentDishes: XCT_LSF_Order = {};
    public orderCheckResult: XCT_LSF_OrderCheckResult = this.initOrderCheckResult();
  ...
  isShowingShopPanel: boolean = false; // 是否 正在展示商店面板
    ...
    // 初始化订单检查结果
    private initOrderCheckResult(): XCT_LSF_OrderCheckResult {
        return {
            extraIngredients: [],
            missingIngredients: [],
    ...
    onLoad() {
          this.addListener();
        XCT_LSF_DataManager.Instance = this;
        //配置数据
        this.ingredientsConfig = this.ingredientsConfigJson.json as XCT_LSF_IngredientsConfig[];
        this.ingredientsConfigObject = this.ingredientsConfig.reduce((obj, item) => {
            obj[item.name] = item;
            return obj;
        }, {} as { [key: string]: XCT_LSF_IngredientsConfig });

        // 提示配置
        this.tipConfig = this.TipConfigJson.json as { [key: string]: string };
        // 外卖订单配置
        this.takeoutOrderConfig = this.TakeoutOrderConfigJson.json as XCT_LSF_DayTakeoutConfig[];
    ...
    * 加载玩家数据，如果没有则初始化默认数据
    */
    private loadData(): XCT_LSF_PlayerData {
        ...
        return {
            money: 100,
            currentDay: 1,
            stock: { "玉米排骨锅": 1, "宽粉": 1, "葱花": 1, "咸鸭蛋": 1, "酱": 1, "盐": 1 },
            score: 5,
    ...
    resetDay() {
        ...
        EventManager.Scene.emit(XCT_Events.LSF_Update_Smile);
        EventManager.Scene.emit(XCT_Events.LSF_Update_Time);
        EventManager.Scene.emit(XCT_Events.LSF_Hide_New_Takeout_Order);

        EventManager.Scene.emit(XCT_Events.LSF_Start_NewDay_Dialog);
    }
     ...
     * @returns 生成的外卖订单配置数组
     */
    private generateRandomTakeoutOrders(): XCT_LSF_TakeoutOrderConfig[] {
        ...
        const selectedOrders: XCT_LSF_TakeoutOrderConfig[] = [];
    ...
    resetCustomer(customer: XCT_LSF_Customer) {
    ...
    resetTakout(orderData: XCT_LSF_TakeoutOrder){
    ...
    passDay() {
        ...
        XCT_UIManager.Instance.showPanel(XCT_UIPanel.LSF_GameUI, null, () => {
            ...
            XCT_UIManager.Instance.showPanel(XCT_UIPanel.LSF_DialoguePanel, null, () => {
                ...
                this.scheduleOnce(() => {
                    //预加载游戏面板
                    XCT_UIManager.Instance.showPanel(XCT_UIPanel.LSF_GamePanel, null, null, XCT_PanelAnimation.PreLoad_at_Bottom);
                    //隐藏开始、加载面板
                    ...
                    XCT_UIManager.Instance.hidePanel(XCT_UIPanel.LoadingPanel, () => {
                        ...
                        // 开始新一天对话
                        this.resetDay();
                        // XCT_DialogManager.Instance.startNewDayDialog(XCT_LSF_DataManager.Instance.playerData.currentDay)
                    });
                }, 0.5);
    ...
    startCooking() {
        if (this.isTakeout) {
            ...
            EventManager.Scene.emit(XCT_Events.LSF_Hide_smile)
        } else {
            this.currentSellingPrice = this.calculateSellingPrice(this.currentCustomer.order);
            EventManager.Scene.emit(XCT_Events.LSF_Show_smile)
        }
        ...
        EventManager.Scene.emit(XCT_Events.LSF_Update_Money);
        XCT_UIManager.Instance.showPanel(XCT_UIPanel.LSF_GamePanel);
        //新手引导
        if (XCT_LSF_DataManager.Instance.playerData.currentDay == 1 && !XCT_LSF_DataManager.Instance.isTutorialEnd)
            XCT_UIManager.Instance.showPanel(XCT_UIPanel.LSF_TutorialPanel, null, null, XCT_PanelAnimation.SLIDE_FROM_BOTTOM);

        XCT_UIManager.Instance.hidePanel(XCT_UIPanel.LSF_DialoguePanel);
        XCT_UIManager.Instance.hidePanel(XCT_UIPanel.LSF_TakeoutPanel, null, XCT_PanelAnimation.SLIDE_TO_TOP);
        this.isCooking = true;
        this.needShowTakeoutCome = false; // 是否需要展示外卖订单结果
          this.isPause = false;
    ...
    endCooking(bowlNode:Node) {
        ...
        XCT_UIManager.Instance.showPanel(XCT_UIPanel.LSF_DialoguePanel, true, () => {
                         EventManager.Scene.emit(XCT_Events.LSF_Show_Bowl,bowlNode);
            if (this.isTakeout) {
                EventManager.Scene.emit(XCT_Events.LSF_Show_Takeout_End_Dialog);
                 EventManager.Scene.emit(XCT_Events.LSF_Show_Dish_Node);
            }
            else {
                 EventManager.Scene.emit(XCT_Events.LSF_Show_End_Dialog_Sp);
                EventManager.Scene.emit(XCT_Events.LSF_Show_Dish_Node);
            }
            this.isEndingDialog = true;

            XCT_UIManager.Instance.hidePanel(XCT_UIPanel.LSF_GamePanel);
    ...
    checkoutCustomer() {
        ...
        if (this.currentSellingAddTip > 0) {
            EventManager.Scene.emit(XCT_Events.LSF_Show_Add_Tip);
    ...
    checkoutDay() {
        ...
        XCT_UIManager.Instance.showPanel(XCT_UIPanel.LoadingPanel, () => {
            XCT_UIManager.Instance.showPanel(XCT_UIPanel.LSF_CheckoutPanel);
        });
    ...
    updateLogic() {
        ...
        for (let i = this.currentDayTakeoutConfig.length - 1; i >= 0; i--) {
            ...
            if ((this.currentHour > orderHour) ||
                ...
                // 创建新的外卖订单对象
                const takeoutOrder: XCT_LSF_TakeoutOrder = {
                    id: Date.now(),
                    ...
                    price: 0, // 初始价格设为0
                    status: XCT_LSF_OrderStatus.UNHANDLED,
                ...
                EventManager.Scene.emit(XCT_Events.LSF_ORDER_DATA_UPDATED);
                if (this.isCooking) {
                    this.needShowTakeoutCome = true; // 是否需要展示外卖订单结果
                }
                else {
                    EventManager.Scene.emit(XCT_Events.LSF_New_Takeout_Order);
                }
                // 从原配置中删除该订单
        ...
        for (let i = 0; i < this.currentTakeoutOrders.length; i++) {
            ...
            // 只处理未处理状态的订单
            if (order.status === XCT_LSF_OrderStatus.UNHANDLED) {
                ...
                if ((this.currentHour > expireHour) ||
                    ...
                    // 将订单状态设置为超时
                    order.status = XCT_LSF_OrderStatus.TIMEOUT;
                    EventManager.Scene.emit(XCT_Events.LSF_ORDER_DATA_UPDATED);
                }
            }
        }
    ...
    // 检查库存是否满足订单
    checkStock(order: XCT_LSF_Order): boolean {
    ...
    // 检查库存是否满足订单
    checkTakeoutStock(order: XCT_LSF_Order): string[] {
        // let order = this.currentCustomer.order;
    ...
    calculateSellingPrice(order: XCT_LSF_Order): number {
        ...
        for (let ingredientName in order) {
            ...
            if (ingredientRequire.type == XCT_LSF_IngredientType.SPICE) {
                sellingPrice = ingredientConfig.sellingPrice * ingredientRequire.count;
            }
            else if (ingredientRequire.type == XCT_LSF_IngredientType.SPREAD) {
                sellingPrice = ingredientConfig.sellingPrice * ingredientRequire.percentage;
            }
            else if (ingredientRequire.type == XCT_LSF_IngredientType.EGG) {
                sellingPrice = ingredientConfig.sellingPrice * ingredientRequire.count;
            }
    ...
    pack() {
        ...
        //     });
        //     if(!XCT_LSF_DataManager.Instance.currentDishes[doughType]){
        //         XCT_LSF_DataManager.Instance.currentDishes[doughType] = {
        //                 type: XCT_LSF_IngredientType.EGG,
        //                 need: 1,
        //                 count: 0,
        //                 percentage: 0,
        //                 rotateCount: 0
        ...
        Object.keys(this.sauceTypeCount).forEach(sauceType => {
            ...
            if (!XCT_LSF_DataManager.Instance.currentDishes[sauceType]) {
                XCT_LSF_DataManager.Instance.currentDishes[sauceType] = {
                    type: XCT_LSF_IngredientType.SPREAD,
                    need: 1,
                    count: 1,
                    percentage: 0,
                    rotateCount: 0
    ...
    // 检查订单
    public checkOrder(order: XCT_LSF_Order) {
        let actualIngredients: XCT_LSF_Order = this.currentDishes;
    ...
    private checkSpiceIngredient(
        ingredient: string,
        orderItem: XCT_LSF_IngredientRequire,
        actualItem: any,
        result: XCT_LSF_OrderCheckResult
    ) {
    ...
    private calculateRefund(order: XCT_LSF_Order) {
        this.refund = 0;
