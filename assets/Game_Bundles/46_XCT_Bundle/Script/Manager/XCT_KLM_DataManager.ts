...
// 配料类型枚举
export enum XCT_KLM_IngredientType {
    SPICE = "撒料类",
    SPREAD = "涂抹类",
    EGG = "鸡蛋类"
}
...
// 按钮回调类型枚举
export enum XCT_KLM_ButtonAction {
    START_MAKE = "开始制作",
    NEXT_DIALOG = "下一段对话",
    BUY_INGREDIENT = "购买食材",
    REFUSE = "拒绝",
...
// 配料要求接口
export interface XCT_KLM_IngredientRequire {
    type: XCT_KLM_IngredientType;
    need: 0 | 1;
    count?: number;
    percentage?: number;
    rotateCount?: number;
...
// 订单接口
export interface XCT_KLM_Order {
    [key: string]: XCT_KLM_IngredientRequire; // 配料名: 要求
}

// 对话段落接口
export interface XCT_KLM_DialogSegment {
    id: number;
    buttons: XCT_KLM_DialogButton[];
    checkStock: boolean;
    text: string;
}

// 对话按钮接口
export interface XCT_KLM_DialogButton {
    text: string;
    action: XCT_KLM_ButtonAction;
    nextSegment: number; // -1:无 0:默认++ 其他:指定编号
}

// 订单检查结果接口
export interface XCT_KLM_OrderCheckResult {
...
// 客人接口
export interface XCT_KLM_Customer {
    id: number;
    avatar: string;
    order: XCT_KLM_Order;
    startDialogs: XCT_KLM_DialogSegment[];
    endDialogs: XCT_KLM_DialogSegment[];
}

// 天数配置接口
export interface XCT_KLM_DayConfig {
    day: number;
    customers: XCT_KLM_Customer[];
}


export interface XCT_KLM_PlayerData {
    money: number;
    currentDay: number;
...
export interface XCT_KLM_IngredientsConfig {
    name: string;
    price: number;
    cost: number,
    sellingPrice: number
...
export enum XCT_KLM_OrderStatus {
    UNHANDLED = 0,    // 未处理
    TIMEOUT = 1,      // 已超时
    COOKED = 2,       // 制作完成
    DELIVERED = 3,     // 已送达
...
// 单个订单数据接口
export interface XCT_KLM_TakeoutOrderConfig {
    id: string;               // 订单唯一标识
    ...
    content: string;          // 订单内容
    remark: string;           // 备注信息
    order: XCT_KLM_Order;            // 订单配置
}

// 外卖订单天数配置接口
export interface XCT_KLM_DayTakeoutConfig {
    day: number;
    takeoutOrders: XCT_KLM_TakeoutOrderConfig[];
}

// 单个订单数据接口
export interface XCT_KLM_TakeoutOrder {
    id: number;               // 订单唯一标识
    avatarName: string;        // 头像资源路径
    customerName: string;     // 点单人昵称
    expireTime: number[];       // 过期时间(如[14,00])
    ...
    price: number;            // 订单价格
    order: XCT_KLM_Order;            // 订单配置
    status: XCT_KLM_OrderStatus;      // 订单状态
    missingStockIngredients: string[];
}

// 数据管理单例
@ccclass('XCT_KLM_DataManager')
export class XCT_KLM_DataManager extends Component {
    public static Instance: XCT_KLM_DataManager;
    private STORAGE_KEY = "XCT_KLM_PlayerData"
    ...
    // 商店配置
    public ingredientsConfig: XCT_KLM_IngredientsConfig[] = null!;
    public ingredientsConfigObject: { [key: string]: XCT_KLM_IngredientsConfig } = null!;

    public tipConfig: { [key: string]: string } = null!;

    // 外卖订单配置
    public takeoutOrderConfig: XCT_KLM_DayTakeoutConfig[] = null!;

    public standardSpreadCount: number = 300;


    // 游戏数据
    public playerData: XCT_KLM_PlayerData = null;
    ...
    public currentCustomer: XCT_KLM_Customer = null;
    ...
    // 外卖订单相关
    public currentDayTakeoutConfig: XCT_KLM_TakeoutOrderConfig[] = [];
    public currentTakeoutOrders: XCT_KLM_TakeoutOrder[] = []; // 当前处理的订单列表
        public lastTakeoutOrderIds: number[] = []; // 最近处理的订单ID列表
    public currentTakeoutOrder: XCT_KLM_TakeoutOrder = null; // 当前处理的订单
    ...
    //制作结果相关
    public currentDishes: XCT_KLM_Order = {};
    public orderCheckResult: XCT_KLM_OrderCheckResult = this.initOrderCheckResult();
 ...
 isShowingShopPanel: boolean = false; // 是否 正在展示商店面板
    ...
    isPacked: boolean = false; // 是否打包
 isPause: boolean = false; // 是否暂停时间更新

    // public stock: { [key: string]: number } = {"鸡蛋":1,"沙拉酱":1}; // 库存

    // 初始化订单检查结果
    private initOrderCheckResult(): XCT_KLM_OrderCheckResult {
        return {
            extraIngredients: [],
            missingIngredients: [],
    ...
    onLoad() {
              this.addListener();
        XCT_KLM_DataManager.Instance = this;
        //配置数据
        this.ingredientsConfig = this.ingredientsConfigJson.json as XCT_KLM_IngredientsConfig[];
        this.ingredientsConfigObject = this.ingredientsConfig.reduce((obj, item) => {
            obj[item.name] = item;
            return obj;
        }, {} as { [key: string]: XCT_KLM_IngredientsConfig });

        // 提示配置
        this.tipConfig = this.TipConfigJson.json as { [key: string]: string };
        // 外卖订单配置
        this.takeoutOrderConfig = this.TakeoutOrderConfigJson.json as XCT_KLM_DayTakeoutConfig[];
    ...
    * 加载玩家数据，如果没有则初始化默认数据
    */
    private loadData(): XCT_KLM_PlayerData {
        ...
        return {
            money: 100,
            currentDay: 1,
            stock: { "鸡蛋": 1, "冷面皮": 1, "油": 1, "洋葱": 1, "浅色酱": 1, "生菜": 1 },
            score: 5,
    ...
    resetDay() {
        ...
        EventManager.Scene.emit(XCT_Events.KLM_Update_Smile);
        EventManager.Scene.emit(XCT_Events.KLM_Update_Time);
        EventManager.Scene.emit(XCT_Events.KLM_Hide_New_Takeout_Order);

        EventManager.Scene.emit(XCT_Events.KLM_Start_NewDay_Dialog);
    }
     ...
     * @returns 生成的外卖订单配置数组
     */
    private generateRandomTakeoutOrders(): XCT_KLM_TakeoutOrderConfig[] {
        ...
        const selectedOrders: XCT_KLM_TakeoutOrderConfig[] = [];
    ...
    resetCustomer(customer: XCT_KLM_Customer) {
    ...
    resetTakout(orderData: XCT_KLM_TakeoutOrder){
    ...
    passDay() {
        ...
        XCT_UIManager.Instance.showPanel(XCT_UIPanel.KLM_GameUI, null, () => {
            ...
            XCT_UIManager.Instance.showPanel(XCT_UIPanel.KLM_DialoguePanel, null, () => {
                ...
                this.scheduleOnce(() => {
                    //预加载游戏面板
                    XCT_UIManager.Instance.showPanel(XCT_UIPanel.KLM_GamePanel, null, null, XCT_PanelAnimation.PreLoad_at_Bottom);
                    //隐藏开始、加载面板
                    ...
                    XCT_UIManager.Instance.hidePanel(XCT_UIPanel.LoadingPanel, () => {
                        ...
                        // 开始新一天对话
                        this.resetDay();
                        // XCT_DialogManager.Instance.startNewDayDialog(XCT_KLM_DataManager.Instance.playerData.currentDay)
                    });
                }, 0.5);
    ...
    startCooking() {
        if (this.isTakeout) {
            ...
            EventManager.Scene.emit(XCT_Events.KLM_Hide_smile)
        } else {
            this.currentSellingPrice = this.calculateSellingPrice(this.currentCustomer.order);
            EventManager.Scene.emit(XCT_Events.KLM_Show_smile)
        }
        ...
        EventManager.Scene.emit(XCT_Events.KLM_Update_Money);
        XCT_UIManager.Instance.showPanel(XCT_UIPanel.KLM_GamePanel);
        //新手引导
        if (XCT_KLM_DataManager.Instance.playerData.currentDay == 1 && !XCT_KLM_DataManager.Instance.isTutorialEnd)
            XCT_UIManager.Instance.showPanel(XCT_UIPanel.KLM_TutorialPanel, null, null, XCT_PanelAnimation.SLIDE_FROM_BOTTOM);

        XCT_UIManager.Instance.hidePanel(XCT_UIPanel.KLM_DialoguePanel);
        XCT_UIManager.Instance.hidePanel(XCT_UIPanel.KLM_TakeoutPanel, null, XCT_PanelAnimation.SLIDE_TO_TOP);
        this.isCooking = true;
        this.needShowTakeoutCome = false; // 是否需要展示外卖订单结果
               this.isPause = false;
    ...
    endCooking() {
        ...
        XCT_UIManager.Instance.showPanel(XCT_UIPanel.KLM_DialoguePanel, true, () => {
            if (this.isTakeout) {
                EventManager.Scene.emit(XCT_Events.KLM_Show_Takeout_End_Dialog);
                  EventManager.Scene.emit(XCT_Events.KLM_Show_Dish_Node);
            }
            else {
                 EventManager.Scene.emit(XCT_Events.KLM_Show_End_Dialog_Sp);
                EventManager.Scene.emit(XCT_Events.KLM_Show_Dish_Node);
            }
            this.isEndingDialog = true;

            XCT_UIManager.Instance.hidePanel(XCT_UIPanel.KLM_GamePanel);
    ...
    checkoutCustomer() {
        ...
        if (this.currentSellingAddTip > 0) {
            EventManager.Scene.emit(XCT_Events.KLM_Show_Add_Tip);
    ...
    updateLogic() {
        ...
        for (let i = this.currentDayTakeoutConfig.length - 1; i >= 0; i--) {
            ...
            if ((this.currentHour > orderHour) ||
                ...
                // 创建新的外卖订单对象
                const takeoutOrder: XCT_KLM_TakeoutOrder = {
                                   id: Date.now(),
                    ...
                    price: 0, // 初始价格设为0
                    status: XCT_KLM_OrderStatus.UNHANDLED,
                ...
                EventManager.Scene.emit(XCT_Events.KLM_ORDER_DATA_UPDATED);
                if (this.isCooking) {
                    this.needShowTakeoutCome = true; // 是否需要展示外卖订单结果
                }
                else {
                    EventManager.Scene.emit(XCT_Events.KLM_New_Takeout_Order);
                }
                // 从原配置中删除该订单
        ...
        for (let i = 0; i < this.currentTakeoutOrders.length; i++) {
            ...
            // 只处理未处理状态的订单
            if (order.status === XCT_KLM_OrderStatus.UNHANDLED) {
                ...
                if ((this.currentHour > expireHour) ||
                    ...
                    // 将订单状态设置为超时
                    order.status = XCT_KLM_OrderStatus.TIMEOUT;
                    EventManager.Scene.emit(XCT_Events.KLM_ORDER_DATA_UPDATED);
                }
            }
        }
    ...
    // 检查库存是否满足订单
    checkTakeoutStock(order: XCT_KLM_Order): string[] {
        // let order = this.currentCustomer.order;
    ...
    calculateSellingPrice(order: XCT_KLM_Order): number {
        ...
        for (let ingredientName in order) {
            ...
            if (ingredientRequire.type == XCT_KLM_IngredientType.SPICE) {
                sellingPrice = ingredientConfig.sellingPrice * ingredientRequire.count;
            }
            else if (ingredientRequire.type == XCT_KLM_IngredientType.SPREAD) {
                sellingPrice = ingredientConfig.sellingPrice * ingredientRequire.percentage;
            }
            else if (ingredientRequire.type == XCT_KLM_IngredientType.EGG) {
                sellingPrice = ingredientConfig.sellingPrice * ingredientRequire.count;
            }
    ...
    pack() {
        ...
        //     });
        //     if(!XCT_KLM_DataManager.Instance.currentDishes[doughType]){
        //         XCT_KLM_DataManager.Instance.currentDishes[doughType] = {
        //                 type: XCT_KLM_IngredientType.EGG,
        //                 need: 1,
        //                 count: 0,
        //                 percentage: 0,
        //                 rotateCount: 0
        ...
        Object.keys(this.sauceTypeCount).forEach(sauceType => {
            ...
            if (!XCT_KLM_DataManager.Instance.currentDishes[sauceType]) {
                XCT_KLM_DataManager.Instance.currentDishes[sauceType] = {
                    type: XCT_KLM_IngredientType.SPREAD,
                    need: 1,
                    count: 1,
                    percentage: 0,
                    rotateCount: 0
    ...
    // 检查订单
    public checkOrder(order: XCT_KLM_Order) {
        let actualIngredients: XCT_KLM_Order = this.currentDishes;
    ...
    private checkSpiceIngredient(
        ingredient: string,
        orderItem: XCT_KLM_IngredientRequire,
        actualItem: any,
        result: XCT_KLM_OrderCheckResult
    ) {
        ...
        // 7. 多放配料（数量过多）
            ...
            let names = ["浅色酱","油","调味酱","花生酱","芝麻酱"]
            if(names.indexOf(ingredient) == -1){
    ...
    private calculateRefund(order: XCT_KLM_Order) {
        this.refund = 0;
