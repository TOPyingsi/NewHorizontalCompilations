...
// 配料类型枚举
export enum XCT_JBT_IngredientType {
    SPICE = "撒料类",
    SPREAD = "涂抹类",
    EGG = "鸡蛋类"
}
...
// 按钮回调类型枚举
export enum XCT_JBT_ButtonAction {
    START_MAKE = "开始制作",
    NEXT_DIALOG = "下一段对话",
    BUY_INGREDIENT = "购买食材",
    REFUSE = "拒绝",
...
// 配料要求接口
export interface XCT_JBT_IngredientRequire {
    type: XCT_JBT_IngredientType;
    need: 0 | 1;
    count?: number;
    percentage?: number;
    rotateCount?: number;
...
// 订单接口
export interface XCT_JBT_Order {
    [key: string]: XCT_JBT_IngredientRequire; // 配料名: 要求
}

// 对话段落接口
export interface XCT_JBT_DialogSegment {
    id: number;
    buttons: XCT_JBT_DialogButton[];
    checkStock: boolean;
    text: string;
}

// 对话按钮接口
export interface XCT_JBT_DialogButton {
    text: string;
    action: XCT_JBT_ButtonAction;
    nextSegment: number; // -1:无 0:默认++ 其他:指定编号
}

// 订单检查结果接口
export interface XCT_JBT_OrderCheckResult {
...
// 客人接口
export interface XCT_JBT_Customer {
    id: number;
    avatar: string;
    order: XCT_JBT_Order;
    startDialogs: XCT_JBT_DialogSegment[];
    endDialogs: XCT_JBT_DialogSegment[];
}

// 天数配置接口
export interface XCT_JBT_DayConfig {
    day: number;
    customers: XCT_JBT_Customer[];
}


export interface XCT_JBT_PlayerData {
    money: number;
    currentDay: number;
    ...
    historyEvaluation: XCT_JBT_SaveTakeoutEvaluation[];
}

export interface XCT_JBT_IngredientsConfig {
    name: string;
    price: number;
    cost: number,
    sellingPrice: number
...
export enum XCT_JBT_OrderStatus {
    UNHANDLED = 0,    // 未处理
    TIMEOUT = 1,      // 已超时
    COOKED = 2,       // 制作完成
    DELIVERED = 3,     // 已送达
...
// 单个订单数据接口
export interface XCT_JBT_TakeoutOrderConfig {
    id: string;               // 订单唯一标识
    ...
    content: string;          // 订单内容
    remark: string;           // 备注信息
    order: XCT_JBT_Order;            // 订单配置
}

// 外卖订单天数配置接口
export interface XCT_JBT_DayTakeoutConfig {
    day: number;
    takeoutOrders: XCT_JBT_TakeoutOrderConfig[];
}

// 单个订单数据接口
export interface XCT_JBT_TakeoutOrder {
    id: number;               // 订单唯一标识
    avatarName: string;        // 头像资源路径
    customerName: string;     // 点单人昵称
    expireTime: number[];       // 过期时间(如[14,00])
    ...
    price: number;            // 订单价格
    order: XCT_JBT_Order;            // 订单配置
    status: XCT_JBT_OrderStatus;      // 订单状态
    missingStockIngredients: string[];
    sellingAddTip:number;
    refund:number;
    star:number;
    evaluation:string;
}
// 单个订单数据接口
export interface XCT_JBT_SaveTakeoutEvaluation {
    avatarName: string;        // 头像资源路径
    customerName: string;     // 点单人昵称
    price: number;            // 订单价格
    refund:number;
...
// 数据管理单例
@ccclass('XCT_JBT_DataManager')
export class XCT_JBT_DataManager extends Component {
    public static Instance: XCT_JBT_DataManager;
    private STORAGE_KEY = "XCT_JBT_PlayerData"
    ...
    @property(JsonAsset)
    EvaluationConfigJson: JsonAsset = null!;

    // @property(JsonAsset)
    // dayMaterialsJson: JsonAsset = null!; // 每日可使用材料配置

    
    // // 每日可使用材料缓存
    // private dayMaterials: {[key: string]: string[]} = {};

    // 商店配置
    public ingredientsConfig: XCT_JBT_IngredientsConfig[] = null!;
    public ingredientsConfigObject: { [key: string]: XCT_JBT_IngredientsConfig } = null!;
    public evaluationConfig: {[key: string]: string[]} = null!;

    public tipConfig: { [key: string]: string } = null!;

    // 外卖订单配置
    public takeoutOrderConfig: XCT_JBT_DayTakeoutConfig[] = null!;

    public standardSpreadCount: number = 100;


    // 游戏数据
    public playerData: XCT_JBT_PlayerData = null;
    ...
    public totalTime: number = 24 * this.MINUTES_PER_HOUR; // 总时间(秒)
    ...
    public currentCustomer: XCT_JBT_Customer = null;
    ...
    // 外卖订单相关
    public currentDayTakeoutConfig: XCT_JBT_TakeoutOrderConfig[] = [];
    public currentTakeoutOrders: XCT_JBT_TakeoutOrder[] = []; // 当前处理的订单列表
    public lastTakeoutOrderIds: number[] = []; // 最近处理的订单ID列表
    public currentTakeoutOrder: XCT_JBT_TakeoutOrder = null; // 当前处理的订单
    public needShowTakeoutCome: boolean = false; // 是否需要展示外卖订单结果

    public notes: string[][] = []; // 顾客订单笔记
    public takeoutNotes: string[][] = []; // 顾客订单笔记

    public evaluationOrder:XCT_JBT_TakeoutOrder = null;


    //制作结果相关
    public currentDishes: XCT_JBT_Order = {};
    public orderCheckResult: XCT_JBT_OrderCheckResult = this.initOrderCheckResult();
    ...
    // 初始化订单检查结果
    private initOrderCheckResult(): XCT_JBT_OrderCheckResult {
        return {
            extraIngredients: [],
            missingIngredients: [],
    ...
    onLoad() {
        this.addListener();
        XCT_JBT_DataManager.Instance = this;
        //配置数据
        this.ingredientsConfig = this.ingredientsConfigJson.json as XCT_JBT_IngredientsConfig[];
        this.ingredientsConfigObject = this.ingredientsConfig.reduce((obj, item) => {
            obj[item.name] = item;
            return obj;
        }, {} as { [key: string]: XCT_JBT_IngredientsConfig });
        // 评价配置
        this.evaluationConfig = this.EvaluationConfigJson.json as {[key: string]: string[]};
        
        // 提示配置
        this.tipConfig = this.TipConfigJson.json as { [key: string]: string };
        // 外卖订单配置
        this.takeoutOrderConfig = this.TakeoutOrderConfigJson.json as XCT_JBT_DayTakeoutConfig[];
    ...
    * 加载玩家数据，如果没有则初始化默认数据
    */
    private loadData(): XCT_JBT_PlayerData {
        ...
        return {
            money: 100,
            currentDay: 1,
            stock: { "鸡蛋": 1, "白面团": 1, "葱花": 1, "调味酱": 1, "薄脆": 1, "生菜": 1 },
            score: 5,
    ...
    resetDay() {
        ...
        EventManager.Scene.emit(XCT_Events.JBT_Update_Smile);
        EventManager.Scene.emit(XCT_Events.JBT_Update_Time);
        EventManager.Scene.emit(XCT_Events.JBT_Hide_New_Takeout_Order);

        EventManager.Scene.emit(XCT_Events.JBT_Start_NewDay_Dialog);
    }
     ...
     * @returns 生成的外卖订单配置数组
     */
    private generateRandomTakeoutOrders(): XCT_JBT_TakeoutOrderConfig[] {
        ...
        const selectedOrders: XCT_JBT_TakeoutOrderConfig[] = [];
    ...
    resetCustomer(customer: XCT_JBT_Customer) {
    ...
    resetTakout(orderData: XCT_JBT_TakeoutOrder){
    ...
    passDay() {
        ...
        XCT_UIManager.Instance.showPanel(XCT_UIPanel.JBT_GameUI, null, () => {
            ...
            XCT_UIManager.Instance.showPanel(XCT_UIPanel.JBT_DialoguePanel, null, () => {
                ...
                this.scheduleOnce(() => {
                    //预加载游戏面板
                    XCT_UIManager.Instance.showPanel(XCT_UIPanel.JBT_GamePanel, null, null, XCT_PanelAnimation.PreLoad_at_Bottom);
                    //隐藏开始、加载面板
    ...
    //  /**
    //  * 根据今日可使用材料和生成数量生成订单数组
    //  * @param day 天数
    //  * @param count 生成订单数量
    //  * @returns 订单数组
    //  */
    // generateOrders(day: number, count: number): XCT_JBT_Order[] {
    //     const orders: XCT_JBT_Order[] = [];
    //     const dayKey = `day_${day}`;
    //     const availableMaterials = this.dayMaterials[dayKey] || [];
        
    //     // 过滤出可用的面团类材料
    //     const doughMaterials = availableMaterials.filter(m => 
    //         m === "白面团" || m === "土豆面"
    //     );
        
    //     // 过滤出酱料类材料
    //     const sauceMaterials = availableMaterials.filter(m => 
    //         m === "调味酱" || m === "辣椒酱"
    //     );
        
    //     const spiceMaterials = availableMaterials.filter(m => 
    //         m === "葱花" || m === "香菜"
    //     );

    //     const doughTypes = ["白面团" ,"土豆面"];
    //     const sauceTypes = ["调味酱" ,"辣椒酱"]

    //     // 过滤出其他材料
    //     const otherMaterials = availableMaterials.filter(m => 
    //         !doughTypes.includes(m) && !sauceTypes.includes(m)
    //     );
        
    //     for (let i = 0; i < count; i++) {
    //         const order: XCT_JBT_Order = {};
            
    //         // 1. 添加面团（白面团或土豆面，二选一）
    //         const dough = doughMaterials[Math.floor(Math.random() * doughMaterials.length)];
    //         order[dough] = {
    //             type: XCT_JBT_IngredientType.SPICE,
    //             need: 1,
    //             count: 1,
    //             rotateCount: this.getRandomFloat(0.8, 2.5, 1) // 随机旋转次数
    //         };
            
    //         // 2. 添加鸡蛋
    //         const eggCount = Math.floor(Math.random() * 5) + 1; // 1-5个
    //         order["鸡蛋"] = {
    //             type: XCT_JBT_IngredientType.SPICE,
    //             need: 1,
    //             count: eggCount
    ...
    //         // 3. 添加酱料（至少一种）
    //         const sauceCount = Math.floor(Math.random() * sauceMaterials.length) + 1;
    //         const selectedSauces = this.getRandomElements(sauceMaterials, sauceCount);
            
    //         selectedSauces.forEach(sauce => {
    //             const count = Math.floor(Math.random() * 2) + 1; // 1-2份
    //             order[sauce] = {
    //                 type: XCT_JBT_IngredientType.SPICE,
    //                 need: 1,
    //                 count: count,
    //                 percentage: this.getRandomFloat(0.3, 1, 1) // 随机百分比
    //             };
    //         });
            
    //         // 4. 添加其他材料（1-5种）
    //         const otherCount = Math.floor(Math.random() * 5) + 1;
    //         const selectedOthers = this.getRandomElements(otherMaterials, otherCount);
            
    //         selectedOthers.forEach((material) => {
    //             // 处理葱花和香菜的特殊情况
    //             const need = Math.random() > 0.3 ? 1 : 0; // 70%概率需要
    ...
    //                 };
    //             } else {
    //                 const count = Math.floor(Math.random() * 5) + 1; // 1-5份
    //                 order[material] = {
    //                     type: XCT_JBT_IngredientType.SPICE,
    //                     need: 1,
    //                     count: count
    ...
    //  * 生成指定范围内的随机浮点数
    //  */
    // private getRandomFloat(min: number, max: number, decimalPlaces: number): number {
    ...
    startCooking() {
        if (this.isTakeout) {
            ...
            EventManager.Scene.emit(XCT_Events.JBT_Hide_smile)
        } else {
            this.currentSellingPrice = this.calculateSellingPrice(this.currentCustomer.order);
            EventManager.Scene.emit(XCT_Events.JBT_Show_smile)
        }
        ...
        EventManager.Scene.emit(XCT_Events.JBT_Update_Money);
        XCT_UIManager.Instance.showPanel(XCT_UIPanel.JBT_GamePanel);
        XCT_AudioManager.getInstance().playMusic("煎");
        //新手引导
        if (XCT_JBT_DataManager.Instance.playerData.currentDay == 1 && !XCT_JBT_DataManager.Instance.isTutorialEnd)
            XCT_UIManager.Instance.showPanel(XCT_UIPanel.JBT_TutorialPanel, null, null, XCT_PanelAnimation.SLIDE_FROM_BOTTOM);

        XCT_UIManager.Instance.hidePanel(XCT_UIPanel.JBT_DialoguePanel);
        XCT_UIManager.Instance.hidePanel(XCT_UIPanel.JBT_TipPanel);
        XCT_UIManager.Instance.hidePanel(XCT_UIPanel.JBT_TakeoutPanel, null, XCT_PanelAnimation.SLIDE_TO_TOP);
        this.isCooking = true;
        this.needShowTakeoutCome = false; // 是否需要展示外卖订单结果
        this.isPause = false;
    ...
    endCooking() {
        ...
        XCT_UIManager.Instance.showPanel(XCT_UIPanel.JBT_DialoguePanel, true, () => {
             XCT_UIManager.Instance.showPanel(XCT_UIPanel.JBT_DialoguePanel, true, () => {
                if (this.isTakeout) {
                    EventManager.Scene.emit(XCT_Events.JBT_Show_Takeout_End_Dialog);
                    this.evaluationOrder = this.currentTakeoutOrder;
                    EventManager.Scene.emit(XCT_Events.JBT_Show_Dish_Node);
                }
                else {
                    EventManager.Scene.emit(XCT_Events.JBT_Show_End_Dialog_Sp);
                    EventManager.Scene.emit(XCT_Events.JBT_Show_Dish_Node);
                }
                this.isEndingDialog = true;

                XCT_UIManager.Instance.hidePanel(XCT_UIPanel.JBT_GamePanel);
            }, XCT_PanelAnimation.SLIDE_FROM_TOP);
        }, XCT_PanelAnimation.PreLoad_at_Top);
        this.isCooking = false;
    ...
    checkoutCustomer() {
        ...
        if (this.currentSellingAddTip > 0) {
            EventManager.Scene.emit(XCT_Events.JBT_Show_Add_Tip);
    ...
    checkoutDay() {
        ...
        XCT_UIManager.Instance.showPanel(XCT_UIPanel.LoadingPanel, () => {
            XCT_UIManager.Instance.showPanel(XCT_UIPanel.JBT_CheckoutPanel);
        });
    ...
    updateLogic() {
        ...
        for (let i = this.currentDayTakeoutConfig.length - 1; i >= 0; i--) {
            ...
            if ((this.currentHour > orderHour) ||
                ...
                // 创建新的外卖订单对象
                const takeoutOrder: XCT_JBT_TakeoutOrder = {
                    id: Date.now(),
                    ...
                    price: 0, // 初始价格设为0
                    status: XCT_JBT_OrderStatus.UNHANDLED,
                ...
                EventManager.Scene.emit(XCT_Events.JBT_ORDER_DATA_UPDATED);
                if (this.isCooking) {
                    this.needShowTakeoutCome = true; // 是否需要展示外卖订单结果
                }
                else {
                    EventManager.Scene.emit(XCT_Events.JBT_New_Takeout_Order);
                }
                // 从原配置中删除该订单
        ...
        for (let i = 0; i < this.currentTakeoutOrders.length; i++) {
            ...
            // 只处理未处理状态的订单
            if (order.status === XCT_JBT_OrderStatus.UNHANDLED) {
                ...
                if ((this.currentHour > expireHour) ||
                    ...
                    // 将订单状态设置为超时
                    order.status = XCT_JBT_OrderStatus.TIMEOUT;
                    EventManager.Scene.emit(XCT_Events.JBT_ORDER_DATA_UPDATED);
                }
            }
        }
    ...
    // 检查库存是否满足订单
    checkTakeoutStock(order: XCT_JBT_Order): string[] {
        // let order = this.currentCustomer.order;
    ...
    calculateSellingPrice(order: XCT_JBT_Order): number {
        ...
        for (let ingredientName in order) {
            ...
            if (ingredientRequire.type == XCT_JBT_IngredientType.SPICE) {
                sellingPrice = ingredientConfig.sellingPrice * ingredientRequire.count;
            }
            else if (ingredientRequire.type == XCT_JBT_IngredientType.SPREAD) {
                sellingPrice = ingredientConfig.sellingPrice * ingredientRequire.percentage;
            }
            else if (ingredientRequire.type == XCT_JBT_IngredientType.EGG) {
                sellingPrice = ingredientConfig.sellingPrice * ingredientRequire.count;
            }
    ...
    pack() {
        ...
        //     });
        //     if(!XCT_JBT_DataManager.Instance.currentDishes[doughType]){
        //         XCT_JBT_DataManager.Instance.currentDishes[doughType] = {
        ...
        Object.keys(this.sauceTypeCount).forEach(sauceType => {
            ...
            if (!XCT_JBT_DataManager.Instance.currentDishes[sauceType]) {
                XCT_JBT_DataManager.Instance.currentDishes[sauceType] = {
                    type: XCT_JBT_IngredientType.SPREAD,
                    need: 1,
                    count: 1,
                    percentage: 0,
                    rotateCount: 0
    ...
    // 检查订单
    public checkOrder(order: XCT_JBT_Order) {
        let actualIngredients: XCT_JBT_Order = this.currentDishes;
    ...
    private checkSpiceIngredient(
        ingredient: string,
        orderItem: XCT_JBT_IngredientRequire,
        actualItem: any,
        result: XCT_JBT_OrderCheckResult
    ) {
    ...
    private calculateRefund(order: XCT_JBT_Order) {
        this.refund = 0;
