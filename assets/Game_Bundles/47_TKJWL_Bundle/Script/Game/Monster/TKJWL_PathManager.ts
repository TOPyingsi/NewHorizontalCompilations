...
import { TKJWL_AStar, TKJWL_AStarNode} from './TKJWL_AStar';
import { TKJWL_Waypoint } from './TKJWL_Waypoint';
import { TKJWL_BinaryHeap } from './TKJWL_BinaryHeap';

// 路径请求接口
interface TKJWL_PathRequest {
    start: TKJWL_Waypoint;
    end: TKJWL_Waypoint;
    callback: (path: TKJWL_Waypoint[] | null, success: boolean) => void;
}

@ccclass('TKJWL_PathManager')
export class TKJWL_PathManager extends Component {
    private aStar = new TKJWL_AStar();
    private pathRequests: TKJWL_PathRequest[] = [];
    private currentPathRequest: TKJWL_PathRequest | null = null;
    ...
    // 单例实例
    public static instance: TKJWL_PathManager;

    // TKJWL_PathManager.ts 新增/修改属性
    private currentAStarState: { // A*分帧计算的状态保存
        openSet: TKJWL_BinaryHeap;
        closedSet: Set<TKJWL_Waypoint>;
        nodeMap: Map<TKJWL_Waypoint, TKJWL_AStarNode>;
        startNode: TKJWL_AStarNode;
        endNode: TKJWL_AStarNode;
    } | null = null;


    onLoad() {
        // if (TKJWL_PathManager.instance) {
        //     this.destroy();
        // } else {
            TKJWL_PathManager.instance = this;
    ...
    // 请求路径
    public requestPath(start: TKJWL_Waypoint, end: TKJWL_Waypoint, callback: (path: TKJWL_Waypoint[] | null, success: boolean) => void) {
        // 取消相同目标的未处理请求
        ...
        const newRequest: TKJWL_PathRequest = {
            start,
            end,
            callback
        };
    ...
    private processPathStep() {
        ...
        if (!this.currentAStarState) {
            ...
            const startNode = new TKJWL_AStarNode(this.currentPathRequest.start);
            const endNode = new TKJWL_AStarNode(this.currentPathRequest.end);
        ...
        let stepCount = 0;
        let path: TKJWL_Waypoint[] | null = null;
        let success = false;
