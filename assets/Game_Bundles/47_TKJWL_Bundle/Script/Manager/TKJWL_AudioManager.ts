...
@ccclass('TKJWL_AudioManager')
export class TKJWL_AudioManager extends Component {
    private static instance: TKJWL_AudioManager;
    ...
    // 单例模式
    public static getInstance(): TKJWL_AudioManager {
        if (!TKJWL_AudioManager.instance) {
            // 查找场景中是否已存在GameManager节点
            const existingNode = director.getScene().getChildByName('GameManager');
            if (existingNode) {
                TKJWL_AudioManager.instance = existingNode.getComponent(TKJWL_AudioManager);
            } else {
                // 如果不存在，则创建新节点并添加组件
                const gameManagerNode = new Node('GameManager');
                director.getScene().addChild(gameManagerNode);
                TKJWL_AudioManager.instance = gameManagerNode.addComponent(TKJWL_AudioManager);
            }
        }
        return TKJWL_AudioManager.instance;
    }

    protected onLoad(): void {
        TKJWL_AudioManager.instance = this;
