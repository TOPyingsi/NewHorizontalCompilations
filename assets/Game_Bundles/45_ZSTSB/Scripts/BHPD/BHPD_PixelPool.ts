...
@ccclass('BHPD_PixelPool')
export class BHPD_PixelPool extends Component {
    ...
    async CreatePixels(num: number) {
        ...
        const effectTarget = BHPD_GameMgr.instance.Target.getChildByName("图片");
    ...
    public recyclePixel() {
        ...
        // const spriteTarget = ZSTSB_GameMgr.instance.Target.getChildByName("图片");
        const maxPoolSize = 2000; // 控制对象池最大容量
        const processBatch = () => {
            // 批量处理
            ...
            for (let i = 0; i < pixelBatch.length; i++) {
                ...
                const pixelComponent = pixel.getComponent(BHPD_Pixel);
                if (pixelComponent) {
