...
@ccclass('XSHY_Joystick')
export class XSHY_Joystick extends Component {
    ...
    onTouchMove_Movement(event: EventTouch) {
        ...
        for (let i = 0; i < touches.length; ++i) {
            ...
            if (this._movementTouch && touch.getID() == this._movementTouch.getID()) {
                ...
                this._scene.emit(XSHY_EasyControllerEvent.MOVEMENT, degree - 90, len / radius);
            }
        }
    }
    ...
    onTouchUp_Movement(event: EventTouch) {
        ...
        for (let i = 0; i < touches.length; ++i) {
            let touch = touches[i];
            if (this._movementTouch && touch.getID() == this._movementTouch.getID()) {
                this._scene.emit(XSHY_EasyControllerEvent.MOVEMENT_STOP);
        ...
        for (let i = 0; i < touches.length; ++i) {
            ...
            if (this._cameraTouchA && this._cameraTouchB) {
                ...
                if (needZoom) {
                    ...
                    this._scene.emit(XSHY_EasyControllerEvent.CAMERA_ZOOM, delta);
            ...
            //only one touch, do camera rotate.
                ...
                this._scene.emit(XSHY_EasyControllerEvent.CAMERA_ROTATE, rx, ry);
            }
        }
    }
    ...
    onKeyDown(event: EventKeyboard) {
        ...
        if (keyCode == KeyCode.KEY_A || keyCode == KeyCode.KEY_D || keyCode == KeyCode.KEY_W || keyCode == KeyCode.KEY_S) {
        ...
        if (keyCode == KeyCode.SPACE) {
            this._scene.emit(XSHY_EasyControllerEvent.JUMP);
        }
    }
    ...
    updateDirection() {
        ...
        if (this._degree == null || this._degree < 0) {
            ...
            this._scene.emit(XSHY_EasyControllerEvent.MOVEMENT, this._degree - 90, 1.0);
        }
    }
}