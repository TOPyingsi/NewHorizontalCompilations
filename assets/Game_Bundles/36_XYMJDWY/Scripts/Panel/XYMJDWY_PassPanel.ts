...
@ccclass('XYMJDWY_PassPanel')
export class XYMJDWY_PassPanel extends Component {
    ...
    //根据赛季经验获刷新当前段位和经验
    ShowData() {
        let seasonExp = XYMJDWY_GameData.Instance.GameData[1];
        ...
        const rankName = XYMJDWY_PassPanel.PassLevel[Math.min(rankIndex, XYMJDWY_PassPanel.PassLevel.length - 1)];
        ...
        this.node.getChildByName("段位奖励").children.forEach((child, index) => {
            if (XYMJDWY_GameData.Instance.SportsCompetitionSeasonData[index] == 0) {
    ...
    OnbuttomClick(btn: EventTouch) {
        ...
        switch (btn.target.name) {
            case "返回":
                ...
                break;
            case "加经验":
                XYMJDWY_GameData.Instance.GameData[1] += 100;
                this.ShowData();
                break;
            case "0":
            case "1":
    ...
    GetAward(Levle: number) {
        ...
        if (XYMJDWY_GameData.Instance.SportsCompetitionSeasonData[Levle] == 0) {
            XYMJDWY_GameData.Instance.SportsCompetitionSeasonData[Levle] = 1;
            XYMJDWY_GameData.Instance.ChanggeGoldBar(this.Award[Levle]);
            if (Levle == 7) {
                XYMJDWY_GameData.Instance.SkinData.push("S1限定头像！");
            }
