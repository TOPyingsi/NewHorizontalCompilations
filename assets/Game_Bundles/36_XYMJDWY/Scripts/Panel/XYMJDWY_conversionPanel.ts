...
@ccclass('XYMJDWY_conversionPanel')
export class XYMJDWY_conversionPanel extends Component {
    ...
    protected onEnable(): void {
        this.node.getChildByName("Content").children.forEach((element, index) => {
            element.active = index == XYMJDWY_conversionPanel.conversionPanelIndex;
        });
    }
    ...
    conversion(demandName: string, demandnum: number, skin: string) {
        if (XYMJDWY_GameData.Instance.SkinData.indexOf(skin) != -1) {
            UIManager.ShowTip("你已拥有改皮肤！");
            return;
        }
        if (XYMJDWY_GameData.Instance.SubKnapsackData(demandName, demandnum)) {
            XYMJDWY_GameData.Instance.SkinData.push(skin);
