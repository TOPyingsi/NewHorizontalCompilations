...
@ccclass('XYMJDWY_Incident')
export class XYMJDWY_Incident extends Component {
    public static LoadSprite(Path: string) {
        return new Promise((resolve, reject) => {
            BundleManager.GetBundle("36_XYMJDWY").load(Path + "/spriteFrame", SpriteFrame, (err, data) => {
                if (err) {
    ...
    public static Loadprefab(Path: string) {
        return new Promise((resolve, reject) => {
            BundleManager.GetBundle("36_XYMJDWY").load(Path, Prefab, (err, data) => {
                if (err) {
