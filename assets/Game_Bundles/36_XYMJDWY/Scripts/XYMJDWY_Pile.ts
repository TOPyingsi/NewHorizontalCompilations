...
import { XYMJDWY_Prop } from './XYMJDWY_Prop';
...
//开堆
@ccclass('XYMJDWY_Pile')
export class XYMJDWY_Pile extends Component {
    ...
    creatProp() {
        ...
        for (let i = 0; i < this.pileMaxContain; i++) {
            ...
            let propQuality: string = XYMJDWY_Constant.QualityType[Math.floor(randomQuality)];
            //随机抽一个稀有度池子
            let qualityPool = XYMJDWY_GameManager.Instance.propPoolMap.get(propQuality);
            //从池子中随机抽一个道具
            ...
            prop.parent = Pile;
            let propTs = prop.getComponent(XYMJDWY_Prop);
    ...
    getProp(index: number) {
        ...
        if (XYMJDWY_GameData.Instance.pushKnapsackData(this.pilePropTsArr[index].propName, 1)) {
            ...
            this.getNum++;
            XYMJDWY_GameData.Instance.GameData[1] += 1;
    ...
    getPool() {
        switch (XYMJDWY_Constant.level) {
            case 1:
                this.qualityPool = Array.from(XYMJDWY_Constant.QualityType);
