...
import { XYMJDWY_Detail } from './XYMJDWY_Detail';
import { XYMJDWY_BagProp } from './XYMJDWY_BagProp';
const { ccclass, property } = _decorator;

@ccclass('XYMJDWY_GameBag')
export class XYMJDWY_GameBag extends Component {
    ...
    start() {
        ...
        for (let i = 0; i < XYMJDWY_GameData.Instance.KnapsackData.length; i++) {
            ...
            this.addPropToBagByName(XYMJDWY_GameData.Instance.KnapsackData[i].Name, 0);
    ...
    openDoor(doorName: string) {
        XYMJDWY_GameData.Instance.SubKnapsackData(doorName, 1);
    ...
    getProp(propName: string, num?: number) {
        let bagData = 0;
        for (let j = 0; j < XYMJDWY_GameData.Instance.KnapsackData.length; j++) {
            if (XYMJDWY_GameData.Instance.KnapsackData[j].Num > 0) {
                bagData++;
            }
        }
        ...
        for (let i = 0; i < bagData - 1; i++) {
            if (XYMJDWY_GameData.Instance.GetPropNum(propName) <= 0) {
                continue;
            }
    ...
    addPropToBagByName(propName: string, num: number) {
        ...
        XYMJDWY_Incident.LoadSprite("Sprites/Prop/" + propName).then((sp: SpriteFrame) => {
            if (this.node?.isValid) {
                ...
                propNode1.getComponent(XYMJDWY_BagProp).propName = propName;
                propNode1.getComponent(XYMJDWY_BagProp).Init();
                ...
                propNode2.getComponent(XYMJDWY_BagProp).propName = propName;
                propNode2.getComponent(XYMJDWY_BagProp).Init();
    ...
    removePropFromBag(propName: string) {
        ...
        XYMJDWY_GameData.Instance.SubKnapsackData(propName, 1);
    ...
    clickProp(propName: string) {
        let _type = XYMJDWY_Constant.GetDataByName(propName).type;
        this.curclickPropName = propName;
        if (_type === "消耗品") {
            switch (propName) {
                case "柑橘辣片":
                    XYMJDWY_GameManager.Instance.player.health(100);

                    break;
                case "香喷喷烤鸡":
                    XYMJDWY_GameManager.Instance.player.health(300);

                    break;
                case "牢大冰红茶":
                    XYMJDWY_GameManager.Instance.player.health(1200);
        ...
        if (_type !== "武器") {
            ...
            let detailTs = this.DetailNode.getComponent(XYMJDWY_Detail);
            let propData = XYMJDWY_Constant.GetDataByName(propName);
    ...
    changeProp(propName: string) {
        ...
        XYMJDWY_Incident.Loadprefab("Prefabs/武器/" + propName).then((prefab: Prefab) => {
            if (this.node?.isValid) {

                let handProp = instantiate(prefab);
                XYMJDWY_GameManager.Instance.player.handProp.destroy();
                handProp.parent = XYMJDWY_GameManager.Instance.player.handRoot;
                XYMJDWY_GameManager.Instance.player.handProp = handProp;
                console.log(handProp.parent);

            }
            XYMJDWY_Constant.GetWeaponDataByName(propName);
