...
import { XYMJDWY_Pile } from './XYMJDWY_Pile';
...
import { XYMJDWY_AIPlayer } from './XYMJDWY_AIPlayer';
import { XYMJDWY_Bullet } from './XYMJDWY_Bullet';
import { XYMJDWY_AudioManager } from './XYMJDWY_AudioManager';
import { ProjectEvent, ProjectEventManager } from 'db://assets/Scripts/Framework/Managers/ProjectEventManager';
import { XYMJDWY_Lock } from './XYMJDWY_Lock';
import { UIManager } from 'db://assets/Scripts/Framework/Managers/UIManager';
import { XYMJDWY_BagProp } from './XYMJDWY_BagProp';
const { ccclass, property } = _decorator;

@ccclass('XYMJDWY_Player')
export class XYMJDWY_Player extends Component {
    ...
    curPlie: XYMJDWY_Pile = null;
    AIArray: XYMJDWY_AIPlayer[] = [];
    onStartContact(selfCollider: Collider2D, otherCollider: Collider2D, contact: IPhysics2DContact | null) {
        ...
        if (group.name === "AIPlayer") {
            ...
            let AITs = group.getComponent(XYMJDWY_AIPlayer);
        ...
        if (group.name === "横锁" || group.name === "竖锁") {
            ...
            let lockTs = otherCollider.node.getComponent(XYMJDWY_Lock);
            let isHasKey = XYMJDWY_GameData.Instance.GetPropNum(lockTs.keyName);
            //碰到锁
        ...
        if (group.name === "BagProp") {
            ...
            this.PickUpBtn.active = true;
            this.propNameSelf = group.getComponent(XYMJDWY_BagProp).propName;
            return;
        }

        let isPlie = group.name.startsWith("Plie");
        if (isPlie) {
            this.curPlie = otherCollider.node.getComponent(XYMJDWY_Pile);
        ...
        if (group.name === "撤离点") {
            XYMJDWY_GameManager.Instance.startExtrac();
        }
    }
    ...
    onExitContact(selfCollider: Collider2D, otherCollider: Collider2D, contact: IPhysics2DContact | null) {
        ...
        if (group === "AIPlayer") {
            if (otherCollider.node.getComponent(XYMJDWY_AIPlayer)) {
                if (this.AIArray.indexOf(otherCollider.node.getComponent(XYMJDWY_AIPlayer)) != -1) {
                    this.AIArray.splice(this.AIArray.indexOf(otherCollider.node.getComponent(XYMJDWY_AIPlayer)), 1);
        ...
        if (group === "撤离点") {
            XYMJDWY_GameManager.Instance.exitExtrac();
        }
    }
    ...
    onMove(Dir: Vec3) {
        ...
        XYMJDWY_Player.couldMove = true;
    }

    onStopMove() {

        XYMJDWY_Player.couldMove = false;
        this._dir = v3(0, 0, 0);

    }

    curLockTs: XYMJDWY_Lock = null;
    PickUp() {
        ...
        XYMJDWY_AudioManager.globalAudioPlay("捡东西");
    ...
    Attack(Dir: Vec3) {
        ...
        this.handPropData = XYMJDWY_Constant.GetDataByName(this.handProp.name);

        if (this.handPropData.type !== "武器" || !this.handPropData) {
            return;
        }

        this.handPropData = XYMJDWY_Constant.GetWeaponDataByName(this.handProp.name);
        ...
        if (propName === "极简武力" || propName === "怒火手枪" || propName === "冰霜神话" || propName === "冲锋枪"
            ...
            XYMJDWY_AudioManager.globalAudioPlay("枪声");

            let bulletNode = instantiate(this.bulletPrefab);

            let bulletTs = bulletNode.getComponent(XYMJDWY_Bullet);
            bulletTs.ATK = attack;
        ...
        XYMJDWY_AudioManager.globalAudioPlay("近战击中");
    ...
    onAttack(damage: number) {
        ...
        if (this.curHealth <= 0) {
            this.curHealth = 0;
            XYMJDWY_Player.couldAttack = false;
            this.isAttacking = false;
            this.node.active = false;
            XYMJDWY_GameData.Instance.KnapsackData = [];
            XYMJDWY_GameManager.Instance.lost();
    ...
    initData() {
        ...
        XYMJDWY_Incident.LoadSprite("Sprites/皮肤/" + XYMJDWY_GameData.Instance.Skin).then((sp: SpriteFrame) => {
            if (this.node?.isValid) {
                this.node.getChildByName("角色图").getComponent(Sprite).spriteFrame = sp;
            }
        });

        XYMJDWY_Incident.Loadprefab("Prefabs/子弹").then((prefab: Prefab) => {
            if (this.node?.isValid) {
        ...
        XYMJDWY_GameManager.Instance.player = this;
    }
