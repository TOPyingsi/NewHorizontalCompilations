...
@ccclass('XYMJDWY_Constant')
export class XYMJDWY_Constant {
    //难度
    ...
    public static GetDataByName(Name: string): { Name: string, type: string, value: number, quality: string, describe: string } {
        for (let i = 0; i < XYMJDWY_Constant.PropData.length; i++) {
            if (XYMJDWY_Constant.PropData[i].Name == Name) {
                return XYMJDWY_Constant.PropData[i];
            }
        }
    ...
    public static GetWeaponDataByName(Name: string): { Name: string, Attack: number, AttackSpeed: number } {
        for (let i = 0; i < XYMJDWY_Constant.WeaponData.length; i++) {
            if (XYMJDWY_Constant.WeaponData[i].Name == Name) {
                return XYMJDWY_Constant.WeaponData[i];
            }
        }
        console.log("名字错误，没有找到对应属性");
        return null;
