...
@ccclass('DWBQB_CheekControl')
export class DWBQB_CheekControl extends Component {
    ...
    @property([SpriteFrame])
    private otherCheek: SpriteFrame[] = [];
    ...
    firstCheekChange(){
        for(let i=0;i<this.cheek.length;i++){
            this.cheek[i]=this.otherCheek[i];
            this.cheek[i]=this.otherCheek[i];
         }
    }
    ...
    oneCheekChange(){
        for(let i=0;i<this.cheek.length;i++){
            this.cheek[i]=this.otherCheek[i+7];
            this.cheek[i]=this.otherCheek[i+7];
         }
    }
    twoCheekChange(){
        for(let i=0;i<this.cheek.length;i++){
            this.cheek[i]=this.otherCheek[i+14];
            this.cheek[i]=this.otherCheek[i+14];
         } 
    }
    threeCheekChange(){
        for(let i=0;i<this.cheek.length;i++){
            this.cheek[i]=this.otherCheek[i+21];
            this.cheek[i]=this.otherCheek[i+21];
         }
    }
