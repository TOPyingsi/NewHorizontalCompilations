...
@ccclass('DWBQB_Peach')
export class DWBQB_Peach extends Component {
    private offset: Vec3 = new Vec3();
    private _isTouching: boolean = false;
    private originalPos: Vec3 = new Vec3();
    private isPlay: boolean = false;
    @property([SpriteFrame])
    private peach: SpriteFrame[] = [];
        ...
        mouthControls.forEach(control => {
            ...
            if (Math.abs(localPos.x) <= mouthTrans.width / 2 &&
                ...
                this.scheduleOnce(() => {
                    ...
                    this.node.getComponent(Sprite).spriteFrame = this.peach[1];
                    this.scheduleOnce(() => {
    ...
    private onTouchEnd(event: EventTouch) {
        ...
        this.scheduleOnce(()=>{
            this.node.getComponent(Sprite).spriteFrame = this.peach[0];
            this.isEat=false;
        },1.5);
