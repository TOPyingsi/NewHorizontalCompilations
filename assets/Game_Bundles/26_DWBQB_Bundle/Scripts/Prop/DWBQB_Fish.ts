...
@ccclass('DWBQB_Fish')
export class DWBQB_Fish extends Component {
    private offset: Vec3 = new Vec3();
    private _isTouching: boolean = false;
    private originalPos: Vec3 = new Vec3();
    private isPlay: boolean = false;
    @property([SpriteFrame])
    private fish: SpriteFrame[] = [];
        ...
        mouthControls.forEach(control => {
            ...
            if (Math.abs(localPos.x) <= mouthTrans.width / 2 &&
                ...
                this.scheduleOnce(() => {
                    ...
                    this.node.getComponent(Sprite).spriteFrame = this.fish[1];
                    this.scheduleOnce(() => {
    ...
    private onTouchEnd(event: EventTouch) {
        ...
        this.scheduleOnce(()=>{
            this.node.getComponent(Sprite).spriteFrame = this.fish[0];
            this.isEat=false;
        },1.5)
