...
@ccclass('DWBQB_Lemon')
export class DWBQB_Lemon extends Component {
    private offset: Vec3 = new Vec3();
    private _isTouching: boolean = false;
    private originalPos: Vec3 = new Vec3();
    @property([SpriteFrame])
    private lemon: SpriteFrame[] = [];
                ...
                if (Math.abs(localPos.x) <= mouthTrans.width / 2 &&
                    Math.abs(localPos.y) <= mouthTrans.height / 2) {
                        ...
                        this.scheduleOnce(()=>{
                            ...
                            this.node.getComponent(Sprite).spriteFrame=this.lemon[1];
                            this.scheduleOnce(()=>{
    ...
    private onTouchEnd(event:EventTouch) {
        ...
        this.scheduleOnce(()=>{
            this.node.getComponent(Sprite).spriteFrame=this.lemon[0];
            this.isEat=false; 
        },1.5);
