...
@ccclass('DWBQB_AnimalMouthControl')
export class DWBQB_AnimalMouthControl extends Component {
    ...
    private _originalPos: Vec3 = new Vec3(0,-197,0);
    ...
    private onTouchMove(event: EventTouch): void {
        ...
        const newScaleY = Math.max(2, Math.min(0.5, 1 - Math.abs(deltaY*0.2) * 0.0002));
        const newScaleX = Math.max(1.2, Math.min(0.5, 1 + Math.abs(deltaY*0.2) * 0.0002));
