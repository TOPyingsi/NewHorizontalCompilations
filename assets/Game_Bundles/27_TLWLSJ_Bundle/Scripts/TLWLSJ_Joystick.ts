...
@ccclass('TLWLSJ_Joystick')
export class TLWLSJ_Joystick extends Component {
    ...
    protected onLoad(): void {
        ...
        this._load = this.node.getChildByName("换弹");
    ...
    OnTouchStart(event: EventTouch) {
        ...
        for (let i = 0; i < touches.length; ++i) {
            ...
            // console.log(x, y, this.node.getComponent(UITransform).width, this.node.getComponent(UITransform).height);
    ...
    OnTouchMove(event: EventTouch) {
        ...
        for (let i = 0; i < touches.length; ++i) {
            ...
            if (this._movementTouch && touch.getID() == this._movementTouch.getID()) {
                ...
                TLWLSJ_EventManager.Scene.emit(TLWLSJ_MyEvent.TLWLSJ_MOVEMENT, dirX, dirY, len / radius);
            }
        }
    }
    ...
    OnTouchEnd(event: EventTouch) {
        ...
        for (let i = 0; i < touches.length; ++i) {
            let touch = touches[i];
            if (this._movementTouch && touch.getID() == this._movementTouch.getID()) {
                TLWLSJ_EventManager.Scene.emit(TLWLSJ_MyEvent.TLWLSJ_MOVEMENT_STOP);
    ...
    OnAttackTouchStart(event: EventTouch) {
        ...
        for (let i = 0; i < touches.length; ++i) {
            ...
            if (!this._attackMovementTouch) {
                ...
                TLWLSJ_EventManager.Scene.emit(TLWLSJ_MyEvent.TLWLSJ_ATTACKSTART);
            }
        }
    }
    ...
    OnAttackTouchMove(event: EventTouch) {
        ...
        for (let i = 0; i < touches.length; ++i) {
            ...
            if (this._attackMovementTouch && touch.getID() == this._attackMovementTouch.getID()) {
                ...
                TLWLSJ_EventManager.Scene.emit(TLWLSJ_MyEvent.TLWLSJ_SET_ATTACK_DIR, dirX, dirY);
            }
        }
    }
    ...
    OnAttackTouchEnd(event: EventTouch) {
        ...
        for (let i = 0; i < touches.length; ++i) {
            ...
            if (this._attackMovementTouch && touch.getID() == this._attackMovementTouch.getID()) {
                ...
                TLWLSJ_EventManager.Scene.emit(TLWLSJ_MyEvent.TLWLSJ_ATTACKEND);
            }
        }
    }

    onReloadTouchEnd(event: EventTouch) {
        TLWLSJ_EventManager.Scene.emit(TLWLSJ_MyEvent.TLWLSJ_RELOAD);
    ...
    updateDir() {
        ...
        TLWLSJ_EventManager.Scene.emit(TLWLSJ_MyEvent.TLWLSJ_MOVEMENT, this._dir.x, this._dir.y, 1.0);
    }
    //#endregion
