...
@ccclass('CPMS_ClearMask')
export default class CPMS_ClearMask extends Mask {
    ...
    Reset() {
        ...
        this.tempDrawPoints = [];
        this.LineWidth = CPMS_GameUI.Instance.lineWidth;
        // 生成小格子，用来辅助统计涂层的刮开比例
    ...
    touchMoveEvent(pos: Vec3, offsetX: number = 0, offsetY: number = 0) {
        ...
        // log("刮开比例" + (this.ClearRate * 100).toFixed(1) + "%")
        if (this.JinDuTiao)
