...
@ccclass('MTRNX_JiGuangJianKonRen')
export class MTRNX_JiGuangJianKonRen extends MTRNX_Unit {
    @property(Prefab)
    Bullet: Prefab = null;
    ...
    public attack: number = 2;//攻击力
    public Hp: number = 110;//当前生命值
    public maxHp: number = 110;//最大生命值
    ...
    update(deltaTime: number) {
        ...
        if (this.TargetNodes.length == 0 && this.State != 2) {
            this.State = 0;
        }
    }
    Attackincident() {
    ...
    Biu() {
        ...
        this.scheduleOnce(() => {
            ...
            if (this.IsBeginAttack && this.TargetNodes.length > 0) {
                MTRNX_AudioManager.AudioClipPlay("蓝色子弹发射", 0.25);
                ...
                let atk = Math.floor(Math.random() * 3 + 1);
                pre.getComponent(MTRNX_WBullet).init(this.TargetNodes[0], atk, this.BulletSpeed, this.IsEnemy, true, linearVelocity);
                pre.setWorldPosition(this.node.getWorldPosition().clone().add(v3(120, 78, 0)));
                this.Biu();
            } else {
                this.IsBeginAttack = false;
