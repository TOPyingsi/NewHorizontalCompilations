import { _decorator, Node, EventTouch, Touch, Component, UITransform, Input, EventKeyboard, KeyCode, v2, Vec3, input, Scene, director, EventMouse, macro, view, screen, Camera, game } from 'cc';
import { MTRNX_EasyControllerEvent } from './MTRNX_EasyController';
...
@ccclass('MTRNX_UI_Joystick')
export class MTRNX_UI_Joystick extends Component {
    ...
    onTouchMove_Movement(event: EventTouch) {
        ...
        for (let i = 0; i < touches.length; ++i) {
            ...
            if (this._movementTouch && touch.getID() == this._movementTouch.getID()) {
                ...
                this._scene.emit(MTRNX_EasyControllerEvent.MOVEMENT, degree - 90, len / radius);
            }
        }
    }
    ...
    onTouchUp_Movement(event: EventTouch) {
        ...
        for (let i = 0; i < touches.length; ++i) {
            let touch = touches[i];
            if (this._movementTouch && touch.getID() == this._movementTouch.getID()) {
                this._scene.emit(MTRNX_EasyControllerEvent.MOVEMENT_STOP);
        ...
        for (let i = 0; i < touches.length; ++i) {
            ...
            if (this._cameraTouchA && this._cameraTouchB) {
                ...
                if (needZoom) {
                    ...
                    this._scene.emit(MTRNX_EasyControllerEvent.CAMERA_ZOOM, delta);
            ...
            //only one touch, do camera rotate.
                ...
                this._scene.emit(MTRNX_EasyControllerEvent.CAMERA_ROTATE, rx, ry);
            }
        }
    }
    ...
    onKeyDown(event: EventKeyboard) {
        ...
        if (keyCode == KeyCode.KEY_A || keyCode == KeyCode.KEY_D) {
        ...
        if (keyCode == KeyCode.SPACE) {
            this._scene.emit(MTRNX_EasyControllerEvent.JUMP);
        }
    }
    ...
    updateDirection() {
        if (this.key2dirMap == null) {
            ...
            this.key2dirMap[KeyCode.KEY_D] = 360;
        ...
        if (this._degree == null || this._degree < 0) {
            ...
            this._scene.emit(MTRNX_EasyControllerEvent.MOVEMENT, this._degree - 90, 1.0);
        }
    }
}