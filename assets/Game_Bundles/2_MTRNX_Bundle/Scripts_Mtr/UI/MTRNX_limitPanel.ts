...
@ccclass('MTRNX_limitPanel')
export class MTRNX_limitPanel extends Component {
    @property(Node)
    Content: Node = null;
    ...
    OnbuttonClick(btn: EventTouch) {
        switch (btn.target.name) {
            case "抽奖":
                this.ChouJiang(btn.target.parent.getComponent(MTRNX_Shop));
                break;
            case "切换":
    ...
    ChouJiang(shopdata: MTRNX_Shop) {
        if (MTRNX_GameDate.Instance.PlayerDate[shopdata.id] == 1) {
            MTRNX_UIManager.HopHint("你已经拥有改角色，无法再次获取！");
            return;
        }
        if (MTRNX_GameDate.Instance.Debris >= shopdata.Debris_price) {
            ...
            if (num < 10) {
                ...
                MTRNX_UIManager.HopHint("成功解锁角色!");
            } else {
                MTRNX_UIManager.HopHint("很遗憾!你没有抽中该角色!");
            }
        } else {
    ...
    //返回
    OnExitClick() {
        MTRNX_UIManager.Instance.HidePanel(MTRNX_Panel.limitPanel);
    }
}
