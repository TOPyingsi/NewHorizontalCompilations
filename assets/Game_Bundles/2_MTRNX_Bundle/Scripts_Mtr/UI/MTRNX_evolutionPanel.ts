...
@ccclass('MTRNX_evolutionPanel')
export class MTRNX_evolutionPanel extends Component {
    @property(Node)
    Content: Node = null;

    Show() {
        this.Content.children.forEach((cd) => {
            if (cd.getComponent(MTRNX_Shop)) {
    ...
    OnbuttonClick(btn: EventTouch) {
        switch (btn.target.name) {
            ...
            case "切换":
                ...
                break;
            case "血量升级":
                if (MTRNX_Include.GetPointIsCan(5000)) {
                    MTRNX_GameDate.Instance.HpLevel += 1;
                    MTRNX_Include.AddPoint(-5000, false);
    ...
    Buy(shopdata: MTRNX_Shop) {
        if (MTRNX_GameDate.Instance.PlayerDate[shopdata.condition] == 0) {
            MTRNX_UIManager.HopHint("解锁改单位需要解锁前置单位！");
            return;
        }
        if (MTRNX_GameDate.Instance.PlayerDate[shopdata.id] == 0) {
            if (MTRNX_GameDate.Instance.Debris >= shopdata.Debris_price) {
                ...
                MTRNX_Include.AddPoint(-shopdata.Debris_price, false);
                this.Show();
            } else {
    ...
    //血量升级显示刷新
    HpLevel_SHow() {
        this.Content.getChildByPath("体力升级/当前").getComponent(Label).string = "当前:" + MTRNX_GameDate.Instance.HpLevel * 50;
    }

    //返回
    OnExitClick() {
        MTRNX_UIManager.Instance.HidePanel(MTRNX_Panel.evolutionPanel);
    }
}
