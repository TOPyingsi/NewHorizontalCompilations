import { _decorator, Component, Node, Vec2, Vec3, instantiate, Prefab, tween, v3, Tween, v2, Color, color } from 'cc';
const { ccclass, property } = _decorator;

@ccclass('MTRNX_ZTool')
export class MTRNX_ZTool extends Component {
    ...
    public static Shaking(node: Node, gap: number, randomMax: number, stopTime: number, callback: Function = null) {
        ...
        tween(node)
            .to(gap, { worldPosition: v3(x + MTRNX_ZTool.GetRandom(-randomMax, randomMax), y + MTRNX_ZTool.GetRandom(-randomMax, randomMax), z) })
            .to(gap, { worldPosition: v3(x + MTRNX_ZTool.GetRandom(-randomMax, randomMax), y + MTRNX_ZTool.GetRandom(-randomMax, randomMax), z) })
            .to(gap, { worldPosition: v3(x + MTRNX_ZTool.GetRandom(-randomMax, randomMax), y + MTRNX_ZTool.GetRandom(-randomMax, randomMax), z) })
            .to(gap, { worldPosition: v3(x + MTRNX_ZTool.GetRandom(-randomMax, randomMax), y + MTRNX_ZTool.GetRandom(-randomMax, randomMax), z) })
            .to(gap, { worldPosition: v3(x + MTRNX_ZTool.GetRandom(-randomMax, randomMax), y + MTRNX_ZTool.GetRandom(-randomMax, randomMax), z) })
    ...
    /**设置 RichText 的颜色 */
    public static RTSetColor(str: string, hex: string): string {
        return `<color=${hex}>${str}</color>`;
    }

    /**设置 RichText 为粗体 */
    public static RTSetBold(str: string): string {
        return `<b>${str}</b>`;
    }

    /**设置 RichText 的大小 */
    public static RTSetSize(str: string, size: number): string {
        return `<size=${size}>${str}</size>`;
    }

    /**设置 RichText 描边 */
    public static RTSetOutline(str: string, hex: string, width: number): string {
        return `<outline color=${hex} width=${width}>${str}</outline>`;
    }
   ...
   */
    ...
    */
    ...
    public static DeepCopy<T>(obj: T) {
        // 如果是基本数据类型或者 null，则直接返回
        if (obj === null || typeof obj !== 'object') {
            return obj;
        }

        // 如果是日期对象，则创建一个新的日期对象并返回
        ...
        if (Array.isArray(obj)) {
            ...
            for (let i = 0; i < obj.length; i++) {
                arrayCopy[i] = MTRNX_ZTool.DeepCopy(obj[i]);
            }
        ...
        for (const key in obj) {
            if (obj.hasOwnProperty(key)) {
                objCopy[key] = MTRNX_ZTool.DeepCopy(obj[key]);
            }
        }
        return objCopy as T;
    }
    ...
    */
    public static checkIsLowPhone() {
        ...
        return Number(MTRNX_ZTool.getBenchmarkLevel()) < checkBenchmark;
    }
