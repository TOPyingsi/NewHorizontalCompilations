import { _decorator, Animation, Collider2D, Component, Contact2DType, IPhysics2DContact, Node, RigidBody2D, v2, v3, Vec2 } from 'cc';
import { MTRNX_PoolManager } from './Utils/MTRNX_PoolManager';
import { MTRNX_Unit } from './MTRNX_Unit';

const { ccclass, property } = _decorator;

@ccclass('MTRNX_WBullet3')
export class MTRNX_WBullet3 extends Component {
    ...
    update(deltaTime: number) {
        ...
        if (this.target?.getComponent(MTRNX_Unit) != null && this.target.getComponent(MTRNX_Unit).Hp > 0) {
            var dir = this.target.getWorldPosition().subtract(this.node.getWorldPosition()).normalize();
