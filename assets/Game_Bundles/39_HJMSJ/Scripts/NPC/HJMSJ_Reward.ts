...
@ccclass('HJMSJ_Reward')
export class HJMSJ_Reward extends Component {



    start() {
        director.getScene().on("哈基米世界_交互", (InteractTarget: string) => {
            if (InteractTarget === "等级奖励") {
                this.open();
            }
        }, this);
    }
    ...
    open() {
        ...
        this.node.getChildByName("奖励界面").active = true;
    ...
    close() {
        ...
        tween(uiOp)
            ...
            .call(() => {
                ...
                this.node.getChildByName("奖励界面").active = false;
            })
            .start();
    }
