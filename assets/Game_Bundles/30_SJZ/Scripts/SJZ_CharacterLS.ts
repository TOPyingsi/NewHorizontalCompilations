...
@ccclass('SJZ_CharacterLS')
export default class SJZ_CharacterLS extends SJZ_CharacterController {
    ...
    onLoad() {
        ...
        // this.SetAniMix(PlayerAniState.LS_Idle, PlayerAniState.LS_Move);
        // this.SetAniMix(PlayerAniState.LS_Move, PlayerAniState.LS_Shoot);
        // this.SetAniMix(PlayerAniState.LS_Shoot, PlayerAniState.LS_Idle);
    }
    ...
    RefreshEquip() {
        let weaponData = SJZ_DataManager.ItemDataMap.get(SJZ_ItemType.Weapon).find(e => e.Name == "S12K");
        //5.56x45mm M9
    ...
    ani: PlayerAniState = PlayerAniState.LS_Idle;
    ...
    update(dt) {
        ...
        if (this.results && this.results.length >= 1 && this.results[0].collider.node.getComponent(RigidBody2D).group == SJZ_Constant.Group.Player) {
            ...
            if (v3_1.length() < 300) {
                ...
                this.ani = PlayerAniState.LS_Move;
                this.Fire();
            }
        ...
        if (this.needMove) {
            ...
            this.PlayAni(0, PlayerAniState.LS_Idle);
            this.rigidbody.linearVelocity = Vec2.ZERO;
        }
    }
    ...
    Fire() {
        if (!this.isFire) {
            if (SJZ_GameManager.IsGameOver) return;
            this.isFire = true;
            this.schedule(this.StartFire, 1);
        }
    }
    ...
    StartFire() {
        ...
        console.error("射击");
        this.PlayAni(1, PlayerAniState.LS_Shoot);
        ...
        for (let i = 0; i < bulletCount; i++) {
            ...
            let recoil = Tools.GetRandom(0.2 * -10 * bulletCount, 0.2 * 10 * bulletCount);

            Bullet.angle = this.character.scale.x < 0 ? 180 - this.gunBone.rotation + recoil : this.gunBone.rotation + recoil;
