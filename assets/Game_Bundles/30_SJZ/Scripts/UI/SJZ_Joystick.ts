...
@ccclass('SJZ_Joystick')
export class SJZ_Joystick extends Component {
    ...
    OnTouchMove(event: EventTouch) {
        ...
        for (let i = 0; i < touches.length; ++i) {
            ...
            if (this._movementTouch && touch.getID() == this._movementTouch.getID()) {
                ...
                EventManager.Scene.emit(SJZ_Constant.Event.MOVEMENT, dirX, dirY, len / radius);
            }
        }
    }
    ...
    OnTouchEnd(event: EventTouch) {
        ...
        for (let i = 0; i < touches.length; ++i) {
            let touch = touches[i];
            if (this._movementTouch && touch.getID() == this._movementTouch.getID()) {
                EventManager.Scene.emit(SJZ_Constant.Event.MOVEMENT_STOP);
    ...
    OnAttackTouchStart(event: EventTouch) {
        let touches = event.getTouches();
        EventManager.Scene.emit(SJZ_Constant.Event.FIRE_START);
    ...
    OnAttackTouchMove(event: EventTouch) {
        ...
        for (let i = 0; i < touches.length; ++i) {
            ...
            if (this._attackMovementTouch && touch.getID() == this._attackMovementTouch.getID()) {
                ...
                EventManager.Scene.emit(SJZ_Constant.Event.SET_ATTACK_DIR, v2(dirX, dirY));
            }
        }
    }
    OnAttackTouchEnd(event: EventTouch) {
        let touches = event.getTouches();
        EventManager.Scene.emit(SJZ_Constant.Event.FIRE_STOP);
    ...
    updateDirection() {
        ...
        EventManager.Scene.emit(SJZ_Constant.Event.MOVEMENT, this._dir.x, this._dir.y, 1.0);
    }
    //#endregion
}