import { _decorator, BoxCollider2D, Collider, Collider2D, Component, Contact2DType, director, EventTouch, ICollisionEvent, IPhysics2DContact, Layers, Node, ScrollView, v2, v3, Vec2, Vec3 } from 'cc';
import { KKDKF_GameManager } from './KKDKF_GameManager';
const { ccclass, property } = _decorator;

@ccclass('KKDKF_MoveTouch')
export class KKDKF_MoveTouch extends Component {
    public IsOnUp: boolean = true;//选中后是否显示在最顶层
    ...
    public IsEnable: boolean = true;//是否激活
    ...
    OnTouchStar(TouchData: EventTouch) {
        KKDKF_GameManager.Instance.MainCanvase.getChildByName("制作区").getComponent(ScrollView).enabled = false;
    ...
    OnTouchEnd(TouchData: EventTouch) {
        KKDKF_GameManager.Instance.MainCanvase.getChildByName("制作区").getComponent(ScrollView).enabled = true;
        if (!this.IsEnable) { return; }
