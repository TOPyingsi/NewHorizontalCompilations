...
import { KKDKF_PoolManager } from './Utils/KKDKF_PoolManager';
import { KKDKF_ResourceUtil } from './Utils/KKDKF_ResourceUtil';


const { ccclass, property } = _decorator;

export enum KKDKF_Panel {
    PrivacyPanel = "Prefabs/PrivacyPanel",
    LoadingPanel = "Prefabs/LoadingPanel",
    SelectGamePanel = "Prefabs/SelectGamePanel",
}

@ccclass('KKDKF_UIManager')
export class KKDKF_UIManager extends Component {
    private static _instance: KKDKF_UIManager = null;
    public static get Instance() {
        if (!this._instance) {
            this._instance = new KKDKF_UIManager();
        }
        return this._instance;
    }
    protected onLoad(): void {
        KKDKF_UIManager._instance = this;
    }
    start() {
        KKDKF_AudioManager.AudioSource = KKDKF_UIManager.Instance.node.getComponent(AudioSource);
    }
    ...
    public static HopHint(str: string) {
        resources.load("Prefabs/提示", Prefab, (err, data) => {
            let pre = KKDKF_PoolManager.Instance.GetNode(data, KKDKF_UIManager.Instance.node.getChildByName("Canvas"));
            pre.getComponent(Label).string = str;
            pre.active = true;
            pre.setPosition(v3(0, 0, 0));
            tween(pre)
                .to(1, { position: v3(0, 150, 0) })
                .call(() => {
                    KKDKF_PoolManager.Instance.PutNode(pre);
                })
                .start();
        }
        )
    ...
    public ShowPanel(panelPath: KKDKF_Panel, args?: any, cb?: Function) {
        ...
        KKDKF_ResourceUtil.LoadUI(panelPath, (err: any, node: any) => {
            //判断是否有可能在显示前已经被关掉了？
