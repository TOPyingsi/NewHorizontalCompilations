...
import { DiggingAHole_Prison_CubeManager } from './DiggingAHole_Prison_CubeManager';
...
@ccclass('DiggingAHole_Prison_CubeGroup')
export class DiggingAHole_Prison_CubeGroup extends Component {
    ...
    Init(pos: Vec3, a: number, b: number) {
                ...
                for (let y = 0; y < 24; y++) {
                    ...
                    for (let z = 0; z < 24; z++) {
                        if (x == 0 || x == 10 || z == 0 || z == 23) {
                            let cube: Node = PoolManager.GetNodeByPrefab(DiggingAHole_Prison_CubeManager.Instance.cubePrefab, this.node.children[0]);
        ...
        for (let i = 0; i < treasLength; i++) {
            let randomTrea = randomRangeInt(0, DiggingAHole_Prison_CubeManager.Instance.treasPrefab.length);
            let trea: Node = PoolManager.GetNodeByPrefab(DiggingAHole_Prison_CubeManager.Instance.treasPrefab[randomTrea], this.node.children[1]);
    ...
    Dig(cube: Node) {
        ...
        if (data.hp <= 0 + parseInt(localStorage.getItem("DAHCV_Dig"))) {
            ...
            DiggingAHole_Prison_Audio.Instance.PlayAudio("dig");
        }
        else DiggingAHole_Prison_Audio.Instance.PlayAudio("dig2");
    }
        ...
        for (let i = x - 1; i < x + 2; i++) {
            for (let j = y - 1; j < y + 2; j++) {
                for (let k = z - 1; k < z + 2; k++) {
                    ...
                    if (j < 0) {
                        let cubeGroup = DiggingAHole_Prison_CubeManager.Instance.cubeGroups.find((value, index, obj) => {
                            if (value.groupPos.y == this.groupPos.y - 1) return value;
                        });
                        if (!cubeGroup) DiggingAHole_Prison_CubeManager.Instance.Init(this.groupPos.y - 1, x, z);
                        else {
                    ...
                    if (j > 23) {
                        ...
                        if (!cubeGroup && this.groupPos.y != 0) DiggingAHole_Prison_CubeManager.Instance.Init(this.groupPos.y - 1, x, z);
                        else if (cubeGroup) {
                            cubeGroup.CheckSingleDig(x, 23, z);
                        }
                        continue;
                    ...
                    let cube = PoolManager.GetNodeByPrefab(DiggingAHole_Prison_CubeManager.Instance.cubePrefab, this.node.children[0]);
                    cube.setPosition(v3(i, j, k));
                    cube.getComponent(Collider).enabled = true;
    ...
    DigTreasure(trea: Node) {
        ...
        DiggingAHole_Prison_GameUI.Instance.GetTrea(data.type);
        PoolManager.PutNode(trea);
        this.treasures.splice(ind, 1);
        let playerData = DiggingAHole_Prison_PlayerController.Instance.Treasures;
        playerData[data.type]++;
        DiggingAHole_Prison_PlayerController.Instance.Treasures = playerData;
    }
