...
import { XYMJ_GameBag } from './XYMJ_GameBag';
const { ccclass, property } = _decorator;

@ccclass('XYMJ_GameManager')
export class XYMJ_GameManager extends Component {
    ...
    public player: XYMJ_Player = null;
    ...
    @property(XYMJ_GameBag)
    public gameBagTs: XYMJ_GameBag = null;

    public static Instance: XYMJ_GameManager = null;
    ...
    start() {
        ...
        XYMJ_GameManager.Instance = this;
        this.initPropData();
    ...
    updateValue() {
        ...
        for (let i = 0; i < XYMJ_GameData.Instance.KnapsackData.length; i++) {
            ...
            let propData = XYMJ_Constant.GetDataByName(XYMJ_GameData.Instance.KnapsackData[i].Name);
            let propValue = propData.value;
            let propNum = XYMJ_GameData.Instance.KnapsackData[i].Num;
        ...
        if (this.isFirst) {
            ...
            let valueStr = "当前物品价值：" + XYMJ_Incident.GetMaxNum(this.totalValue);
            this.valueLabel.string = valueStr;
            return;
        }

        this.newValue = this.totalValue - this.startValue;
        let valueStr = "当前物品价值：" + XYMJ_Incident.GetMaxNum(this.newValue);
        this.valueLabel.string = valueStr;
    ...
    win() {
        ...
        XYMJ_GameData.Instance.GameData[1] += 50;
        ...
        tween(this.winNode)
            ...
            .call(() => {
                ...
                value.active = true;
                value.getComponent(Label).string = "撤离物品价值：" + XYMJ_Incident.GetMaxNum(this.newValue);
    ...
    //初始化道具数据
    initPropData() {
        for (let j = 0; j < XYMJ_Constant.QualityType.length; j++) {
            this.propPoolMap.set(XYMJ_Constant.QualityType[j], []);
        }

        for (let i = 0; i < XYMJ_Constant.PropData.length; i++) {
            let quality = XYMJ_Constant.PropData[i].quality;
            let data = XYMJ_Constant.PropData[i];
            if (data.type === "回收物") {
        ...
        XYMJ_Incident.Loadprefab("Prefabs/场景/" + XYMJ_Constant.mapID).then((prefab) => {
            ...
            let playerTs = curMap.getChildByName("Player").getComponent(XYMJ_Player);
            playerTs.initData();
        });
