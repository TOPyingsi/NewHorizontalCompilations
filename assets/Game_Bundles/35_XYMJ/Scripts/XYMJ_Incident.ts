...
@ccclass('XYMJ_Incident')
export class XYMJ_Incident extends Component {
    public static LoadSprite(Path: string) {
        return new Promise((resolve, reject) => {
            BundleManager.GetBundle("35_XYMJ").load(Path + "/spriteFrame", SpriteFrame, (err, data) => {
                if (err) {
    ...
    public static Loadprefab(Path: string) {
        return new Promise((resolve, reject) => {
            BundleManager.GetBundle("35_XYMJ").load(Path, Prefab, (err, data) => {
                if (err) {
