...
import XGTW_Broadcast from '../UI/XGTW_Broadcast';

@ccclass('XGTW_UIManager')
export class XGTW_UIManager extends Component {
    @property(JsonAsset)
    SceneDate: JsonAsset | null = null;
    private _panelDict: any = {}
    private _loadingPanelDict: any = {}
    private _arrPopupDialog: any = []
    private static _instance: XGTW_UIManager;
    public static get Instance() {
        if (!this._instance) {
            this._instance = new XGTW_UIManager();
        }

        return this._instance;
    }
    protected onLoad(): void {
        XGTW_UIManager._instance = this;
    }
    ...
    public ShowPanel(panelPath: string, args?: any, cb?: Function, panelPriority: number = XGTW_Constant.PRIORITY.NORMAL) {
        if (this._loadingPanelDict[panelPath]) {
            return;
        }

        let idxSplit = panelPath.lastIndexOf('/');
        let scriptName = `XGTW_` + panelPath.slice(idxSplit + 1);
        ...
        // if(panelPath == XGTW_Constant.Panel.WinPanel || panelPath == XGTW_Constant.Panel.FailPanel){//结算
    ...
    */
    ...
    public static ShowBroadcast(content: string, path: string, position: Vec2, delay: number = 0.75, tweenType: number = 2) {
        PoolManager.GetNodeByBundle(GameManager.GameData.DefaultBundle, XGTW_Constant.Path.Broadcast, find("Canvas")).then(node => node.getComponent(XGTW_Broadcast).Show(content, path, position, delay, tweenType));
    }
}